...
| Feed de dados | Campo XDM | Tipo XDM | Descrição |
| — | — | — | — |
| `post_evar1`<br/>`[...]`<br/>`post_evar250` | `_experience.analytics.customDimensions.`<br/>`eVars.eVar1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`eVars.eVar250` | cadeia de caracteres | eVars de Análise Personalizada. Cada organização pode usar eVars de forma diferente. |
| `post_prop1`<br/>`[...]`<br/>`post_prop75` | `_experience.analytics.customDimensions.`<br/>`props.prop1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`props.prop75` | cadeia de caracteres | Propriedades personalizadas do Analytics. Cada organização pode usar props de forma diferente. |
...
| `post_campaign` | `marketing.trackingCode` | cadeia de caracteres | A variável usada na dimensão Código de rastreamento. |
| `post_channel` | `web.webPageDetails.siteSection` | cadeia de caracteres | A variável usada na dimensão Seções do site. |
| `post_cust_visid` | `endUserIDs._experience.aacustomid.id` | cadeia de caracteres | A ID de visitante personalizada, se definida. |
| `post_first_hit_page_url` | `_experience.analytics.endUser.`<br/>`firstWeb.webPageDetails.URL` | sequência de caracteres | A URL da primeira página que o visitante atinge. |
...
| `post_keywords` | `search.keywords` | sequência de caracteres | As palavras-chave que foram coletadas para a ocorrência. |
| `post_page_url` | `web.webPageDetails.URL` | cadeia de caracteres | A URL da ocorrência da página. |
...
| `post_purchaseid` | `commerce.order.purchaseID` | cadeia de caracteres | Variável usada para identificar compras de maneira exclusiva. |
| `post_referrer` | `web.webReferrer.URL` | cadeia de caracteres | A URL da página anterior. |
| `post_state` | `_experience.analytics.customDimensions.`<br/>`stateProvince` | cadeia de caracteres | Variável de estado. |
| `post_user_server` | `web.webPageDetails.server` | cadeia de caracteres | Uma variável usada na dimensão Servidor. |
| `post_zip` | `_experience.analytics.customDimensions.`<br/>`postalCode` | cadeia de caracteres | Uma variável usada para preencher a dimensão CEP. |
| `browser` | `_experience.analytics.environment.`<br/>`browserID` | inteiro | A ID numérica do navegador. |
| `domain` | `environment.domain` | cadeia de caracteres | A variável usada na dimensão Domínio. Ele se baseia no provedor de serviços de Internet (ISP) do usuário. |
| `first_hit_referrer` | `_experience.analytics.endUser.`<br/>`firstWeb.webReferrer.URL` | sequência de caracteres | A primeira URL de referência do visitante. |
| `geo_city` | `placeContext.geo.city` | cadeia de caracteres | O nome da cidade da ocorrência. Isso se baseia no endereço IP da ocorrência. |
| `geo_dma` | `placeContext.geo.dmaID` | inteiro | A ID numérica da área demográfica da ocorrência. Isso se baseia no endereço IP da ocorrência. |
| `geo_region` | `placeContext.geo.stateProvince` | cadeia de caracteres | O nome do estado ou da região da ocorrência. Isso se baseia no endereço IP da ocorrência. |
| `geo_zip` | `placeContext.geo.postalCode` | cadeia de caracteres | O CEP da ocorrência. Isso se baseia no endereço IP da ocorrência. |
...
| `search_page_num` | `search.pageDepth` | inteiro | Essa variável é usada pela dimensão Todas as classificações da página de pesquisa e indica em qual página de resultados de pesquisa seu site | apareceu antes de o usuário clicar no site. |
| `visit_keywords` | `_experience.analytics.session.`<br/>`search.keywords` | cadeia de caracteres | Uma variável usada na dimensão Palavras-chave de pesquisa. |
...
| `visit_referrer` | `_experience.analytics.session.`<br/>`web.webReferrer.URL` | cadeia de caracteres | O primeiro referenciador da visita. |
| `visit_search_page_num` | `_experience.analytics.session.`<br/>`search.pageDepth` | inteiro | O primeiro Nome de página da visita. |
| `post_prop1`<br/>`[...]`<br/>`post_prop75` | `_experience.analytics.customDimensions.`<br/>`listprops.prop1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`listprops.prop75` | Objeto | Props de análise personalizadas, configuradas para serem propriedades de lista. Ela contém uma lista delimitada de valores. |
| `post_hier1`<br/>`[...]`<br/>`post_hier5` | `_experience.analytics.customDimensions.`<br/>`hierarchies.hier1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`hierarchies.hier5` | Objeto | Usado por variáveis de hierarquia e contém uma lista delimitada de valores. | {valores (matriz), delimitador (cadeia de caracteres)} |
| `post_mvvar1`<br/>`[...]`<br/>`post_mvvar3` | `_experience.analytics.customDimensions.`<br/>`lists.list1.list[]`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`lists.list3.list[]` | matriz | Uma lista de valores de variáveis. Contém uma lista delimitada de valores personalizados, dependendo da implementação. | {value (string), key (string)} |
...
| `post_java_enabled` | `environment.browserDetails.javaEnabled` | booleano | Um sinalizador que indica se o Java™ está habilitado. |
...
| `post_page_event_var1` | `web.webInteraction.URL` | cadeia de caracteres | Essa variável é usada somente em solicitações de imagem de rastreamento de link. É o URL do link de download, link de saída, ou link personalizado clicado. |
| `post_page_event_var2` | `web.webInteraction.name` | cadeia de caracteres | Essa variável é usada somente em solicitações de imagem de rastreamento de link. É o nome personalizado do link. |
| `post_page_type` | `web.webPageDetails.isErrorPage` | booleano | Usado para preencher a dimensão Páginas não encontradas. Essa variável deve estar vazia ou conter &quot;ErrorPage&quot; |
...
| `post_product_list` | `productListItems[].items` | matriz | A lista de produtos, conforme transmitido pela variável products. | {SKU (sequência de caracteres), quantidade (número inteiro), priceTotal (número)} |
...
| `mvvar1_instances` | `.list.items[]` | Objeto | Lista de valores de variáveis. Contém uma lista delimitada de valores personalizados, dependendo da implementação. |
| `mvvar2_instances` | `.list.items[]` | Objeto | Lista de valores de variáveis. Contém uma lista delimitada de valores personalizados, dependendo da implementação. |
| `mvvar3_instances` | `.list.items[]` | Objeto | Lista de valores de variáveis. Contém uma lista delimitada de valores personalizados, dependendo da implementação. |
...
| `visit_ref_type` | `_experience.analytics.session.`<br/>`web.webReferrer.type` | cadeia de caracteres | ID numérica que representa o tipo do primeiro referenciador da visita. |
| `visit_search_engine` | `_experience.analytics.session.`<br/>`search.searchEngine` | cadeia de caracteres | ID numérica do primeiro mecanismo de pesquisa da visita. |
