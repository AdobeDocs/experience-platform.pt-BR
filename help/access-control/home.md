...
O Controle de acesso for Experience Platform é fornecido pelo [Adobe Admin Console](https://adminconsole.adobe.com). Essa funcionalidade aproveita perfis de produtos no Admin Console, que vinculam usuários com permissões e caixas de proteção.

## Hierarquia de Controles de acesso e fluxo de trabalho

Para configurar o controle de acesso para a plataforma Experience, você deve ter privilégios de administrador para uma organização que tenha uma integração de produto da plataforma Experience. A função mínima que concede ou retira permissões é um administrador **de perfil de** produto. Outras funções de administrador que podem gerenciar permissões são administradores **de** produtos (podem gerenciar todos os perfis em um produto) e administradores **de** sistema (sem restrições). Consulte o artigo da Central de ajuda da Adobe sobre funções [](https://helpx.adobe.com/enterprise/using/admin-roles.html) administrativas para obter mais informações.

>[!NOTE] A partir desse ponto, quaisquer menções de &quot;administrador&quot; neste documento se referem a um administrador do perfil do produto ou a uma versão superior (conforme descrito acima).
...
- Quando os usuários fazem logon na interface do usuário da plataforma Experience, seu acesso aos recursos da plataforma é determinado pelas permissões que lhes foram concedidas na Etapa 2. Por exemplo, se um usuário não tiver a permissão &quot;Conjuntos de dados de Visualização&quot;, a guia *Conjuntos* de dados no menu lateral não estará visível para esse usuário.

Para obter etapas mais detalhadas sobre como gerenciar o controle de acesso na plataforma Experience, consulte o guia [do usuário do](./ui/overview.md)controle de acesso.

Todas as chamadas para APIs da plataforma Experience são validadas para obter permissões e retornarão erros se as permissões apropriadas não forem encontradas no contexto do usuário atual. Na interface do usuário, os elementos serão omitidos ou alterados, dependendo das permissões concedidas ao usuário atual.

## Adobe Admin Console

O Adobe Admin Console fornece um local central para gerenciar os direitos e o acesso dos produtos da Adobe para sua organização. Por meio do console, você pode conceder a grupos de usuários permissões de acesso para vários recursos da plataforma, como &quot;Gerenciar conjuntos de dados&quot;, &quot;Conjuntos de dados de Visualização&quot; ou &quot;Gerenciar Perfis&quot;.
...
A plataforma Experience vem com dois perfis de produto padrão pré-configurados. A tabela a seguir descreve o que é fornecido em cada perfil padrão, incluindo a caixa de proteção à qual eles concedem acesso, bem como as permissões que eles concedem dentro do escopo dessa caixa de proteção.

| perfil do produto | Acesso à caixa de proteção | Permissões |
| --- | --- | --- |
| Produção padrão - Todos os acessos | Produção | Todas as permissões se aplicam à plataforma Experience, exceto as permissões de Administração da caixa de proteção. |
...
A plataforma Experience fornece acesso a uma caixa de proteção Production (Produção) e permite criar **caixas de proteção Non-Production (Não produção)**. As caixas de proteção de não-produção são uma forma de virtualização de dados que permite isolar dados de outras caixas de proteção e geralmente são usadas para experimentos de desenvolvimento, testes ou testes. As **permissões** de um perfil de produto concedem aos usuários do perfil acesso aos recursos da Plataforma dentro dos ambientes da caixa de proteção aos quais eles receberam acesso.

Para obter mais informações sobre caixas de proteção na plataforma Experience, consulte a visão geral [das](../sandboxes/home.md)caixas de proteção.
...
A tabela a seguir descreve as permissões disponíveis para a plataforma Experience no Admin Console, com descrições dos recursos específicos da plataforma aos quais eles concedem acesso. Para obter etapas detalhadas sobre como adicionar permissões a um perfil de produto, consulte o guia [do usuário do](./ui/overview.md)controle de acesso.

| Categoria | Permissão | Descrição |
| --- | --- | --- |
| Modelagem de dados | Gerenciar Schemas | Acesso para ler, criar, editar e excluir schemas e recursos relacionados. |
| Modelagem de dados | Schemas Visualizações | Acesso somente leitura a schemas e recursos relacionados. |
...
| Gestão de Dados | Conjuntos de dados de Visualização | Acesso somente leitura para conjuntos de dados e schemas. |
...
| Administração de sandbox | Caixas de proteção de Visualização | Acesso somente leitura para caixas de proteção pertencentes à sua organização. |
...
_(*) Esta permissão exige provisões para a Plataforma de dados do cliente em tempo real. Para obter mais informações sobre a CDP em tempo real, comece lendo a visão geral[da CDP em tempo](https://docs.adobe.com/content/help/en/experience-platform/rtcdp/overview.html)real._

## Próximas etapas

Ao ler este guia, você foi apresentado aos principais princípios do controle de acesso na plataforma Experience. Agora você pode continuar com o guia [do usuário do](./ui/overview.md) controle de acesso para obter etapas detalhadas sobre como usar o Admin Console para criar perfis de produtos e atribuir permissões para a Plataforma.