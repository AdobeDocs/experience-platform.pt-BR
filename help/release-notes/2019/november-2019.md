---
title: Notas de versão da Adobe Experience Platform novembro de 2019
description: As notas de versão de novembro de 2019 do Adobe Experience Platform.
...
Incorporada na Adobe Experience Platform, a Real-time Customer Data Platform (CDP em tempo real) ajuda as empresas a unir dados conhecidos e desconhecidos para ativar perfis do cliente com decisões inteligentes em toda a jornada do cliente. A CDP em tempo real combina várias fontes de dados corporativas para criar perfis unificados em tempo real, que podem ser usados para fornecer experiências personalizadas individuais do cliente em todos os canais e dispositivos.
...
Para obter mais informações, consulte o [Documentação do Real-time Customer Data Platform](../../rtcdp/overview.md).

**Principais recursos**

| Recurso | Descrição |
|---|---|
| Destinos | Integrações pré-criadas com plataformas de destino compatíveis com o Adobe [!DNL Real-time Customer Data Platform] que ativam os dados para esses parceiros de forma contínua. Consulte [Destinos](#destinations) abaixo para obter mais informações. |
| Painel de métricas da página inicial | A página inicial da Real-time Customer Data Platform (CDP em tempo real) inclui um painel de métricas que mostra informações sobre perfis e segmentos. A página inicial também contém links para materiais de aprendizagem. Consulte a seção sobre [Métricas do Real-time Customer Data Platform](#real-time-customer-data-platform-metrics) abaixo. |
| Fontes | Você pode assimilar dados de várias fontes, como Soluções Adobe, armazenamento baseado em nuvem, software de terceiros e seu CRM. Consulte a [Fontes](#sources) seção abaixo para saber mais. |
...
[!DNL Destinations] são integrações pré-criadas com plataformas de destino compatíveis com o Adobe Real-time Customer Data Platform que ativam os dados para esses parceiros de forma contínua. Para obter mais informações, leia a [Visão geral dos destinos](../../destinations/home.md) artigo 10. o

**Destinos disponíveis**

Com a versão de novembro, a Real-time Customer Data Platform suporta os seguintes destinos:
...
* No momento, não há como editar ou excluir uma configuração de destino. Para contornar essa limitação, você pode ativar ou desativar o destino no canto superior direito da [página de detalhes do destino](../../destinations/ui/destination-details-page.md).
...
| Fluxos de trabalho modernizados para conectores CRM | Novo fluxo de trabalho intuitivo da interface do usuário para criar e gerenciar [!DNL Microsoft Dynamics] e [!DNL Salesforce] conectores. |
| Suporte a conector para armazenamento baseado em nuvem | Os conectores agora podem acessar armazenamentos baseados em nuvem. Novas fontes incluem [!DNL Amazon S3], [!DNL Azure Blob]e servidores FTP/SFTP. |
...
| Recurso | Descrição |
| -----------| ---------- |
| Acesso aos dados usando [!DNL Platform] SDK | Fórmulas pré-criadas e notebooks iniciadores em [!DNL Python] use agora [!DNL Platform] SDK para acessar dados. |
...
| Recurso | Descrição |
| -----------| ---------- |
| Melhorias a [!DNL Profile] pesquisa | Os usuários agora podem pesquisar perfis usando descritores de referência e entidades relacionadas. |
| Limpar dados de um determinado conjunto de dados | Agora os usuários podem excluir dados de um determinado conjunto de dados ou lote usando o [!DNL Profile] API de tarefas do sistema. |
| Edge [!DNL Profile] aprimoramentos de query | Os aplicativos agora podem consultar o Edge [!DNL Profile] por qualquer das identidades de um determinado perfil. |
...
| Segmentação de streaming | O suporte para a avaliação contínua dos segmentos (segmentação de fluxo) permite que a maioria das regras de segmento seja avaliada à medida que os dados são transmitidos [!DNL Platform]. Esse recurso significa que a associação de segmento estará atualizada sem a necessidade de executar tarefas de segmentação programadas. Algumas exceções se aplicam, como segmentos que usam relações de várias entidades ou com cargas enriquecidas. |
...
| Suporte aprimorado para compartilhamento de público externo entre [!DNL Platform] e soluções Adobe | Os usuários agora podem trazer para o exterior (não[!DNL Experience Platform]) metadados do público-alvo em cenários em que o número de públicos-alvo é grande ou desconhecido a priori. Esta versão inclui acesso a [!DNL Audience Manager] metadados para clientes que provisionaram o conector da solução. Os metadados do público-alvo podem ser usados no Construtor de segmentos para criar novos [!DNL Experience Platform] segmentos. <br/><br/> Além disso, segmentos criados em [!DNL Experience Platform] O agora estará disponível para uso em soluções Adobe integradas, incluindo [!DNL Audience Manager], [!DNL Target]e [!DNL Ad Cloud]. |
