...
| Destino | Descrição |
| ----------- | ----------- |
| [(Beta) Conexão do Adobe Experience Cloud Audiences](../../destinations/catalog/adobe/experience-cloud-audiences.md) | Use a conexão [!UICONTROL (Beta) Adobe Experience Cloud Audiences] para compartilhar segmentos do Experience Platform com várias soluções da Experience Platform, como Audience Manager, Analytics, Advertising, Campaign, Target ou Marketo. |
...
| --- | --- | --- |
| Classe | [[!UICONTROL Conversion]](https://github.com/adobe/xdm/blob/master/components/classes/conversion.schema.json) | Uma classe para rastrear dados de conversão como conversões de moeda. |
| Grupo de campos | [[!UICONTROL Currency Conversion Rate Details]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/conversion/currency-conversion-details.schema.json) | Um grupo de campos para a classe [!UICONTROL Conversion], capturando detalhes adicionais relacionados à conversão de moeda. |
| Grupo de campos | [[!UICONTROL Consent policies evaluation results map with metadata]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/profile/profile-consentResultsv2.schema.json) | Captura detalhes para o resultado da avaliação de várias políticas de consentimento, incluindo informações de metadados sobre as entradas e saídas da política de consentimento. |

**Componentes de XDM atualizados**

| Tipo de componente | Nome | Descrição |
| --- | --- | --- |
| Tipo de dados | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/advertisingdetails.schema.json) | O campo `ID` foi renomeado para `name`, e o antigo campo `name` agora é `friendlyName`. |
| Tipo de dados | [[!UICONTROL Decision Proposition Details]](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/decisioning/proposition-detail.schema.json) | Um campo `selectionStrategy` que captura os detalhes de uma estratégia de seleção foi adicionado. |
| Grupo de campos | [[!UICONTROL Experience Event - Proposition Interactions]](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/decisioning/experienceevent-proposition-interaction.schema.json) | O grupo de campos agora é compatível com a classe [!UICONTROL Journey Step Event]. |
| Tipo de dados | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/errordetails.schema.json) | O campo `ID` foi renomeado para `name`. |
| Tipo de dados | [[!UICONTROL Media information]](https://github.com/adobe/xdm/blob/master/docs/reference/datatypes/media.schema.json) | Uma alteração no padrão da propriedade de segmento do vídeo foi revertida. |
| Tipo de dados | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/qoedatadetails.schema.json) | Remoção do campo `droppedFrameCount`. |
| Tipo de dados | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/sessiondetails.schema.json) | O campo `isAuthorized` foi renomeado para `authorized` e teve seu `type`, que era um booleano, atualizado para uma string. |
| Tipo de dados | [[!UICONTROL Shipping]](https://github.com/adobe/xdm/blob/master/components/datatypes/shipping.schema.json) | Vários novos campos foram adicionados: `shipDate`, `trackingNumber`, e `trackingURL`. |
| Grupo de campos | [[!UICONTROL AJO Entity Fields]](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/customerJourneyManagement/ajo-entity-mixins.schema.json) | Vários novos campos foram adicionados: `journeyNodeID`, `journeyNodeName`, e `journeyModeType`. |
| Grupo de campos | [[!UICONTROL Consumer Experience Event]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/experienceevent-consumer.schema.json) | O grupo de campos agora também é compatível com a classe [!UICONTROL Summary Metrics]. |
| Grupo de campos | [[!UICONTROL Product Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/product-triggers.schema.json) | O campo `productTriggers` agora está aninhado em um objeto `weather`. |
| Grupo de campos | [[!UICONTROL Relative Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/relative-triggers.schema.json) | O campo `relativeTriggers` agora está aninhado em um objeto `weather`. |
| Grupo de campos | [[!UICONTROL Severe Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/severe-triggers.schema.json) | O campo `severeTriggers` agora está aninhado em um objeto `weather`. |
| Grupo de campos | [[!UICONTROL Weather Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/severe-triggers.schema.json) | O campo `weatherTriggers` agora está aninhado em um objeto `weather`. |
| Grupo de campos | [[!UICONTROL XDM Related Business Accounts]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/account/related-accounts.schema.json) | O grupo de campos agora está estável. |
...
**Nova descontinuação** {#deprecation}

Para remover a redundância no ciclo de vida da associação a um segmento, o status `Existing` será descontinuado do [mapa de segmentos de associação](../../xdm/field-groups/profile/segmentation.md) no final de março de 2023. Um anúncio posterior incluirá a data exata da descontinuação.
...
Essa alteração pode afetar você se estiver usando [destinos corporativos](../../destinations/destination-types.md#advanced-enterprise-destinations) (Amazon Kinesis, Hubs de Eventos do Azure, API HTTP) e possuir processos downstream automatizados em vigor, com base no status `Existing`. Revise suas integrações downstream se esse for o seu caso. Caso se interesse em identificar perfis recém-qualificados além de um determinado período, considere usar uma combinação dos status `Realized` e `lastQualificationTime` no mapa de associação a segmentos. Para obter mais informações, entre em contato com o representante da Adobe.
...
| Expiração de associação de público-alvo externo | Por padrão, as associações de público-alvo externo são mantidas por 30 dias. Para mantê-las por mais tempo, use o campo `validUntil` durante a ingestão de dados do público-alvo. |
| Expiração de associação de segmento gerada pela Experience Platform | Qualquer associação a um segmento que esteja no estado `Exited` por mais de 30 dias, com base no campo `lastQualificationTime`, estará sujeita a exclusão. |
