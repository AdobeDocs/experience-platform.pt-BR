...
## Versão 2.33.1 - 7 de maio de 2026
...
- Correção de um problema em que o Web SDK falhava quando havia vários parâmetros `s_kwcid` ou `ef_id` relacionados a anúncios no URL.
- Correção de um problema em que os dados do Advertising eram enviados e os cookies eram criados antes da obtenção do consentimento.
- Correção de um problema no Safari em que os fluxos do Brand Concierge não eram analisados corretamente.
...
- Agora há suporte para [substituições de sequência de dados](/help/datastreams/overrides.md) ao iniciar sessões de mídia.
...
- Correção de um erro que ocorria quando itens de conteúdo padrão eram renderizados via [`applyPropositions`](commands/applypropositions.md).
- Correção de um erro CSS nas ações de movimentação e redimensionamento do Adobe Target.
- A chave `machineLearning` foi removida das respostas [`sendEvent`](commands/sendevent/overview.md).
...
- Adicionado suporte para [Coleção de mídia de streaming](commands/configure/streamingmedia.md).
...
- Adicionado suporte para [eventos de início e fim de página](../use-cases/personalization/top-bottom-page-events.md).
...
- Adicionado suporte para [substituições por comando da ID de sequência de dados](/help/datastreams/overrides.md).
...
- Adicionado suporte para retorno de chamada de clique de link [`onBeforeLinkClickSend`](commands/configure/onbeforelinkclicksend.md).
...
- Correção de um problema em que as propriedades da [dica do cliente de agente do usuário](../use-cases/client-hints.md) podiam ser definidas como um tipo incorreto.
...
- Adição de suporte para Migração completa de página por página. O perfil do Adobe Target agora será preservado à medida que um visitante se move entre as páginas da at.js e do Web SDK.
- Adição de suporte configurável para [User-Agent Client Hints de alta entropia](../use-cases/client-hints.md).
- Suporte adicionado para o comando [`applyResponse`](commands/applyresponse.md). Isso habilita a personalização híbrida através da [API do Edge Network](https://developer.adobe.com/data-collection-apis/docs/api/).
...
- O SDK agora pode ser instalado como um [pacote NPM](install/npm.md).
- Adicionado suporte para uma opção `out` ao [configurar o consentimento padrão](commands/configure/defaultconsent.md), que descarta todos os eventos até que o consentimento seja recebido (a opção `pending` existente enfileira eventos e os envia quando o consentimento é recebido).
- O retorno de chamada [`onBeforeEventSend`](commands/configure/onbeforeeventsend.md) agora pode ser usado para impedir que um evento seja enviado.
...
- O comando [`getIdentity`](commands/getidentity.md) agora retorna a ID da região da borda ao lado da identidade.
- Os avisos e erros recebidos do servidor foram aprimorados e são tratados de maneira mais apropriada.
- Adição de suporte ao padrão Consentimento 2.0 da Adobe para o comando [`setConsent`](commands/setconsent.md).
