...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md): A estrutura padronizada pela qual o Experience Platform organiza os dados de experiência do cliente.
   ...
   * [API](../../../../xdm/api/getting-started.md) do Registro de Esquema: Saiba como executar chamadas com êxito para a API do Registro de Schema. Isso inclui seu `{TENANT_ID}`, o conceito de &quot;contêineres&quot; e os cabeçalhos necessários para fazer solicitações (com especial atenção ao cabeçalho Accept e seus possíveis valores).
...
| -------- | ----------- |
| `baseConnectionId` | A ID de conexão da sua fonte **[!UICONTROL eCommerce]**. |
| `params.path` | O caminho do arquivo de origem. |
| `connectionSpec.id` | A ID da especificação de conexão da sua fonte **[!UICONTROL eCommerce]**. |
...
Um fluxo de dados é responsável por coletar dados de fontes e trazê-los para [!DNL Platform]. Para criar um fluxo de dados, primeiro obtenha as especificações do fluxo de dados executando uma solicitação do GET para a API [!DNL Flow Service]. As especificações do fluxo de dados são responsáveis pela coleta de dados de uma fonte **[!UICONTROL eCommerce]**.
...
| `scheduleParams.frequency` | O `frequency` no qual o fluxo de dados coletará dados. Os valores aceitáveis incluem: `once`, `minute`, `hour`, `day` ou `week`. |
