...
Além de queries SQL padrão, Adobe Experience Platform [!DNL Query Service] suporta a gravação de consultas usando [!DNL Experience Events]. Os Eventos de experiência são representados pela classe ExperienceEvent do Experience Data Model (XDM), que captura um instantâneo imutável e não agregado do sistema quando um usuário interage com um site ou serviço e, portanto, pode ser usado para análise de domínio de tempo.

Mais informações sobre o XDM e [!DNL Experience Events] podem ser encontradas no [[!DNL XDM System] visão geral](../../xdm/home.md). Ao combinar [!DNL Query Service] com [!DNL Experience Events], você pode efetivamente rastrear tendências comportamentais entre seus usuários. O documento a seguir fornece exemplos de consultas envolvendo [!DNL Experience Events].
...
A coluna de carimbo de data e hora encontrada em [!DNL Experience Event] Os conjuntos de dados estão em UTC. O exemplo a seguir usa a variável `from_utc_timestamp()` para transformar o carimbo de data e hora de UTC em EDT. Em seguida, usa a variável `date_format()` para isolar a data do restante do carimbo de data e hora.
...
Para obter mais informações sobre consultas de exemplo usando funções definidas pelo Adobe (ADFs), leia o guia Funções definidas pelo Adobe. Para obter orientações gerais sobre a execução da consulta, leia a [guia sobre a execução de query no Serviço de query](../best-practices/writing-queries.md).
