...
**Ação:** No Experience Platform Launch, uma ação é um tipo específico de componente de regra que define o que deve ocorrer depois que um evento ocorre e as condições são avaliadas e passadas.

**Ativar:** Na Plataforma de dados do cliente em tempo real, ativar é a ação executada por um usuário para mapear um segmento ou perfis para um destino como Oracle Eloqua, Google ou Salesforce Marketing Cloud.

**Atividade:** No serviço de decisão, uma atividade é um conjunto de ofertas do qual o comerciante deseja que o mecanismo de decisão selecione a melhor oferta.
...
**Conectores da Adobe:** Os Conectores Adobe são conexões pré-configuradas criadas pela Adobe para permitir que os dados fluam para dentro e para fora da plataforma Experience. Os conectores incluem Microsoft Dynamics, Salesforce, Amazon S3 e Azure Blob.

**Serviços inteligentes da Adobe:** O Adobe Sensei é a estrutura de inteligência que alimenta a plataforma de experiência. Ele também fornece um conjunto de serviços de IA que capacita as marcas a melhorar sua capacidade de fornecer experiências personalizadas e em tempo real aos clientes.

**E/S da Adobe:** A E/S da Adobe faz parte da Plataforma de experiência e fornece acesso a tudo que os desenvolvedores precisam para integrar, estender e personalizar a Plataforma de experiência da Adobe, incluindo APIs, eventos, console do desenvolvedor e ferramentas úteis.

**Adobe Sensei:** O Adobe Sensei é a estrutura de inteligência que alimenta a plataforma de experiência. Ele também fornece um conjunto de serviços de IA que capacita as marcas a melhorar sua capacidade de fornecer experiências personalizadas e em tempo real aos clientes.

**Balde Amazon S3:** Os buckets do Amazon S3 são os container fundamentais para dados armazenados no ecossistema da Amazônia. Os buckets contêm objetos, cada objeto é armazenado e recuperado usando uma chave exclusiva atribuída ao desenvolvedor.

**Conector Amazon S3:** O conector Amazon S3 permite que os clientes da Experience Platform conectem e acessem com segurança seus dados Amazon S3.

**Anexar estratégia de gravação:** A estratégia de `Append` salvamento é uma opção usada ao especificar dados de terceiros para assimilar por meio de uma conexão e anexar quaisquer novos dados ou linhas ao final do conjunto de dados. As linhas ingeridas anteriormente permanecem intocadas e somente as linhas criadas desde a última execução programada são ingeridas na plataforma da experiência. Todas as linhas que foram alteradas no sistema de origem permanecem inalteradas na Experience Platform.
...
**Preenchimento retroativo:** Na Plataforma de dados do cliente em tempo real, em conexões de origem programadas, o preenchimento retroativo permite a ingestão de dados históricos.

**Período de preenchimento retroativo:** `Backfill period` é uma opção para definir o tempo necessário para assimilar dados históricos de terceiros por meio de uma conexão. Selecionar um período de preenchimento retroativo para sempre assimilará todo o histórico dos dados de origem à plataforma da experiência.
...
**Ingestão em lote:** A ingestão em lote permite que os usuários ingeram petabytes de dados e os disponibilizem em sistemas corporativos. Com as tecnologias mais recentes, os usuários agora podem assimilar qualquer XDM e não-XDM do schema na Experience Platform.
...
**Compilação:** No Experience Platform Launch, uma compilação é uma biblioteca implantada. A compilação é um arquivo ou conjunto de arquivos que contém todas as configurações e códigos necessários para executar a lógica comercial contida dentro dessa biblioteca.

**Ferramentas de Business Intelligence:** A inteligência empresarial, também conhecida como ferramentas &quot;BI&quot;, é integrada principalmente ao Serviço de Query da plataforma Experience. As ferramentas BI são tipos de software de aplicativos que coletam e processam grandes quantidades de dados não estruturados de sistemas internos e externos.
...
**Cliente:** Um cliente é uma ferramenta ou aplicativo externo que se conecta ao Serviço de Query por meio do protocolo de pôsteres ou da API HTTP.

**Coleção:** No Serviço de tomada de decisão, as coleções são subconjuntos de ofertas com base em condições predefinidas definidas definidas por um comerciante, como a categoria da oferta.

**Interface de linha de comando:** Interface de linha de comando é uma ferramenta de linha de comando usada para conectar-se ao Serviço de Query para execução de query brutos.

**Composição**: Uma composição é um agrupamento de componentes que se formam juntos para formar o schema.

**Conexão:** Uma conexão é um pipeline virtual que permite que os dados fluam para dentro e para fora da plataforma Experience. As conexões agora são substituídas por Fontes.

**Conector:** Os conectores da Fonte da plataforma Adobe Experience ajudam os usuários a assimilar facilmente dados de várias fontes, permitindo a estruturação, a rotulagem e a melhoria de dados usando os Serviços da plataforma Experience. Os dados podem ser assimilados de várias fontes, como armazenamentos baseados em nuvem, software de terceiros e sistemas CRM.
...
**Console:** No Query Service, o console fornece informações sobre o status e a operação de um Query. O console exibe o status da conexão com o Serviço de Query, as operações de query que estão sendo executadas e quaisquer mensagens de erro que resultem desses query.
...
**Dicionário de dados:** No Experience Platform Launch, um dicionário de dados é um conjunto de elementos de dados definidos em uma propriedade.

**Elemento de dados:** No Experience Platform Launch, um elemento de dados é um ponteiro usado dentro de regras e extensões para apontar para uma parte específica dos dados existentes no dispositivo cliente.

**Camada de dados:** No Experience Platform Launch, uma camada de dados é uma estrutura de dados que existe no dispositivo cliente que contém metadados sobre o contexto no qual uma página ou tela é visualizada.
...
**Parceiros de integração de dados:** Os parceiros de integração de dados simplificam e automatizam o carregamento e a transformação de grandes volumes de dados de mais de 200 fontes para a plataforma Experience sem gravar código.
...
**Área de trabalho da ciência de dados:** A Área de trabalho da ciência de dados na plataforma Experience permite que os clientes criem modelos de aprendizado de máquina utilizando dados na plataforma Experience e nos aplicativos Adobe para gerar insights e previsões inteligentes para tecer experiências digitais deliciosas para o usuário final.
...
**Regra de decisão:** No serviço de decisão, uma regra de decisão é a lógica que define e controla o que, quando, onde e como uma oferta é apresentada aos usuários finais.

**Serviço de decisão:** O Serviço de decisão é uma coleção de serviços e interface do usuário que permite aos comerciantes criar e fornecer experiências de oferta personalizadas para o usuário final em canais e aplicativos usando lógica de negócios e regras de decisão.

**Coluna Delta:** Na Plataforma de dados do cliente em tempo real, a coluna delta permite a seleção do campo de dados de origem para um carimbo de data e hora para ingestão incremental

**Estratégia de gravação Delta:** `Delta save strategy` é uma opção para assimilar dados de terceiros por meio de uma conexão. A opção permite que o usuário especifique se linhas novas ou alteradas de dados de origem são ingeridas na Experience Platform. Novas linhas são adicionadas ao final do conjunto de dados e as linhas alteradas são atualizadas no conjunto de dados na plataforma da experiência.

**Destino:** Na Plataforma de dados do cliente em tempo real, um destino é um termo geral para qualquer sistema, como um aplicativo da Adobe, servidor de anúncios ou rede de anúncios em que uma audiência é ativada e entregue.

**Categoria de destino:** Uma categoria de destino é um agrupamento de destinos da Plataforma de dados do cliente em tempo real com características semelhantes.

**Catálogo de destino:** Um catálogo de destino é uma lista de destinos disponíveis na Plataforma de dados do cliente em tempo real.
...
**Oferta elegível:** No serviço de decisão, uma oferta qualificada atende às restrições definidas em upstream que podem ser consistentemente oferecidas a um perfil.

**Regras qualificadas:** No Serviço de decisão, as regras de elegibilidade são aplicadas a um perfil relacionado a restrições de calendário, agendamento e limite.

**Código incorporado:** No Experience Platform Launch, o código incorporado é uma tag de script colocada dentro do HTML em um site ou ambiente. O código incorporado instrui o navegador para onde recuperar a compilação.

**Lista discriminada:** Uma enumeração é uma lista de valores que representam os dados válidos de um campo.

**Ambiente:** No Experience Platform Launch, um ambiente é um conjunto de instruções de implantação que especifica o delivery host e o formato de arquivo de uma compilação. Uma biblioteca deve ser emparelhada a um ambiente antes de ser criada.

**Evento** na plataforma Experience Launch, um evento é um tipo específico de componente de regra, um acionador que ocorre em um dispositivo cliente para iniciar a execução de uma regra.
...
**ExperienceEvent:** O ExperienceEvent é um schema padrão da plataforma de experiência que captura observações, incluindo o ponto no tempo e a identidade do assunto envolvido. Os Eventos de experiência são registros de fatos do que ocorreu, representando o que aconteceu sem agregação ou interpretação.

**Extensão:** No Experience Platform Launch, uma extensão é um pacote de funcionalidade adicionado a uma propriedade Launch.  Geralmente, uma extensão é focada em uma determinada solução de marketing ou análise e fornece as ferramentas necessárias para implantar essa tecnologia em um ambiente cliente.

**Pacote de extensão:** No Experience Platform Launch, um pacote de extensão é um arquivo .zip criado e carregado por um desenvolvedor de extensão que fornece tudo o que é necessário para que os usuários do Launch instalem a extensão dentro de sua propriedade.  Um pacote de extensão contém um manifesto que especifica informações sobre a extensão, o HTML e o JavaScript necessários para que os usuários finais configurem o comportamento da extensão Launch e o JavaScript executável entregue ao ambiente cliente, se necessário.

[Voltar ao início](#adobe-experience-platform-glossary)

## F

**Ofertas de fallback:** No Serviço de decisão, uma oferta de fallback é a oferta padrão exibida quando um usuário final não está qualificado para nenhuma das ofertas na coleção usada.
...
**Frequência:** A frequência determina a frequência com que um query de Serviço de Query programado recorrente será executado.
...
**Host:** No Experience Platform Launch, um host especifica o local, o domínio e as credenciais do usuário necessários para que o Launch forneça uma compilação.
...
**Namespace de identidade:** Uma namespace de identidade é um identificador, como ID de cookie, ID de dispositivo ou ID de email, para indicar o contexto a partir do qual os dados se originam e são usados para reconhecer e vincular identidades na Experience Cloud.

**Serviço de identidade:** A interface do usuário do Serviço de identidade da plataforma Experience permite a criação e o gerenciamento de tipos de identidade para permitir a vinculação de identidades entre dispositivos e canais para uma visualização completa do usuário a partir do Perfil do cliente em tempo real.
...
**Ingest:** A ingestão é o processo de adicionar dados de uma fonte à plataforma da experiência. Os dados podem ser assimilados à plataforma da experiência de várias maneiras, incluindo transmissão em sequência, armazenamento em lote ou adição por meio de conector.

**Agendamento de ingestão:** O agendamento de ingestão fornece opções baseadas em tempo ao assimilar de uma fonte para a plataforma de experiência.
...
**Serviços inteligentes:** Serviços inteligentes, como Attribution.ai e Customer.ai, são aprendizado de máquina, modelos baseados em inteligência artificial que exigem que a plataforma de experiência execute e opere.
...
**JupyterLab:** Uma interface baseada na Web de código aberto para o Project Júpiter e é totalmente integrada à plataforma Experience.

**Notebook de Júpiter:** Um aplicativo da Web de código aberto que permite que os usuários criem e compartilhem documentos que contêm código ativo, equações, visualizações e texto narrativo.
...
**Biblioteca:** No Experience Platform Launch, uma biblioteca é um conjunto de lógicas de negócios que contém instruções sobre como a biblioteca do Launch deve se comportar no dispositivo cliente.
...
**Modelo de aprendizado de máquina:** Um modelo de aprendizado de máquina é uma instância de uma fórmula de aprendizado de máquina que é treinada usando dados históricos e configurações para resolver um caso de uso comercial. Na Adobe Data Science Workspace, os modelos de aprendizado por máquina são chamados de receitas.

**Mapeamento:** Na Plataforma de dados do cliente em tempo real, o mapeamento de dados é o processo de mapeamento dos campos de dados de origem para campos de público alvo relacionados ao destino.

**Ação de marketing:** Uma ação de marketing, no contexto da estrutura de gerenciamento de dados, é uma ação que um consumidor de dados da plataforma de experiência toma, para a qual há necessidade de verificar violações das políticas de uso de dados.
...
**Política de Mesclagem:** Uma política de mesclagem é um conjunto de regras usadas pelo Perfil para determinar como os dados serão priorizados e combinados em uma visualização unificada em determinadas condições.
...
**Módulo:** No Experience Platform Launch, um módulo é um trecho do JavaScript executável fornecido por uma extensão, que executa ações em um ambiente cliente sem a necessidade do usuário do Launch criar uma regra.
...
**Notebooks:** Os notebooks são criados com o Notebook *de Júpiter* e contêm descrição da análise, resultados e podem ser executados para executar a análise de dados.

[Voltar ao início](#adobe-experience-platform-glossary)

## O

**Oferta:** No serviço de decisão, uma oferta é uma mensagem de marketing que pode ter regras associadas a ela que especificam quem está qualificado para ver a oferta.

**Decisão de Oferta:** No serviço de decisão, a decisão de oferta permite que um profissional de marketing gerencie as regras e os modelos treinados de apresentações da oferta ao se envolver com um usuário final com base nos dados coletados em canais e aplicativos.
...
**Ofertas personalizadas:** No serviço de decisão, uma oferta personalizada é uma mensagem de marketing personalizável baseada em regras de elegibilidade e restrições.

**Disposições:** No Serviço de tomada de decisão, uma disposição é o local e/ou contexto em que uma oferta é exibida para um usuário final.

**Política:** Uma política de uso de dados é uma regra que especifica ações de marketing que são restritas com base na aplicação de rótulos de uso de dados nos dados da plataforma Experience.

**Chave primária:** A chave primária é uma designação em um schema para identificar exclusivamente todos os registros.

**Prioridade:** No Serviço de decisão, a prioridade é usada para classificar ofertas que atendem a todas as restrições, como qualificação, calendário e limite.
...
**Perfil do produto:** Os perfis de produtos permitem que os administradores concedam acesso de usuário a todos ou a um subconjunto de serviços associados à plataforma de experiência.
...
**Plataforma de dados do cliente em tempo real:** A Plataforma de dados do cliente em tempo real da Adobe reúne dados conhecidos e desconhecidos do cliente para criar perfis confiáveis do cliente com integração simplificada, segmentação inteligente e ativação em tempo real na jornada do cliente digital.

**Perfil do cliente em tempo real:** O Perfil de cliente em tempo real é um perfil centralizado para gerenciamento de experiência direcionado e personalizado.
...
**Recorrência:** Uma recorrência define se um query do Serviço de Query está programado para ser executado apenas uma vez ou de forma recorrente.

**Representação:** No Serviço de decisão, uma representação é informação usada por um canal, como localização ou idioma para exibir uma oferta.

**Recurso:** No Experience Platform Launch, o recurso é um termo genérico que se refere às opções que o usuário do Launch pode configurar dentro do ambiente cliente, incluindo extensões, elementos de dados e regras.

**Controle de acesso baseado em função:** O controle de acesso baseado em funções permite que os administradores atribuam acesso e permissões a usuários da plataforma de experiência. As permissões incluem a capacidade de visualização e/ou uso de recursos da plataforma de experiência, como criação de caixas de proteção, definição de schemas e gerenciamento de conjuntos de dados.

**Regra:** No Experience Platform Launch, uma regra é uma coleção de componentes de regras que definem um conjunto específico de eventos, condições e ações que devem ser agrupados logicamente.
...
**Caixa de proteção:** Uma caixa de proteção é uma forma de isolar dados virtuais em uma organização de usuários na Experience Platform.
...
**Comutador Sandbox:** O controle alternador de sandbox na plataforma Experience permite que os usuários naveguem entre caixas de proteção às quais têm acesso. Alternar uma caixa de proteção alterará todo o conteúdo e poderá alterar o acesso ao recurso com base em permissões.

**Agendamento:** Agendamento é uma especificação definida pelo usuário sobre a frequência ou cadência da ingestão de dados de uma fonte de dados de terceiros para a Adobe Experience Platform.
...
**Chave de acesso secreta:** Uma chave de acesso secreta é uma chave Amazon S3 usada em conjunto com a ID da chave de acesso para assinar solicitações AWS.
...
**Construtor de segmentos:** O Construtor de segmentos é o ambiente de desenvolvimento visual usado para criar definições de segmentos e serve como um componente comum de todos os aplicativos usando a Segmentação em tempo real do Perfil do cliente na plataforma da experiência.
...
**Tipo de segmento:** Há dois tipos de segmentos: Um é um segmento que é atualizado dinamicamente com as alterações de dados da plataforma de experiência, e o outro é um instantâneo de audiência que captura todas as regras de segmento de reunião de perfis, e elas não são alteradas.
...
**Conector de origem:** Os conectores da Fonte da plataforma Adobe Experience ajudam os usuários a assimilar facilmente dados de várias fontes, permitindo a estruturação, a rotulagem e a melhoria de dados usando os Serviços da plataforma Experience. Os dados podem ser assimilados de várias fontes, como armazenamentos baseados em nuvem, software de terceiros e sistemas CRM.

**Categoria de origem:** Uma categoria de origem é um agrupamento de fontes da Plataforma de dados do cliente em tempo real com características semelhantes.

**Catálogo de origem:** Um catálogo de origem é uma lista de fontes disponíveis na Plataforma de dados do cliente em tempo real.
...
**Ingestão de transmissão:** A assimilação de transmissão contínua fornece aos usuários um método para enviar dados de dispositivos cliente e servidor para a plataforma Experience em tempo real.

**URL do Ponto Final de Transmissão:** Um URL de terminal de transmissão contínua é um terminal exclusivo fornecido pela Adobe e vinculado à organização IMS de um cliente para transmitir dados na plataforma Experience.
...
**Visualização do sistema:** A Visualização do sistema é uma representação visual de conjuntos de dados de origem que fluem pelo Perfil do cliente em tempo real para destinos.
...
**Token:** Um token é um tipo de segurança de autenticação de dois fatores que pode ser usado para autorizar o uso de serviços de computador com o Serviço de Query.
...
**Schema da União:** O União é uma consolidação de schemas que foram habilitados para o Perfil do cliente em tempo real.
