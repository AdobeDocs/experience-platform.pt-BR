---
keywords: Experience Platform;home;popular topics;crm schema;crm;CRM;dataflow;Dataflow;;home;popular topics;crm ;crm;CRM;dataflow;Dataflow
solution: Experience Platform
title: Configurar um fluxo de dados para uma conexão de origem CRM na interface do usuário
topic: overview
type: Tutorial
...
Um fluxo de dados é uma tarefa programada que recupera e ingere dados de uma fonte para um conjunto de dados [!DNL Platform]. Este tutorial fornece etapas para configurar um novo fluxo de dados usando sua conta CRM.
...
Além disso, este tutorial requer que você já tenha criado uma conta CRM. Uma lista de tutoriais para criar conectores CRM diferentes na interface do usuário pode ser encontrada na [visão geral dos conectores de origem](../../../home.md).
...
| Backfill | Um valor booliano que determina quais dados são inicialmente assimilados. Se **[!UICONTROL Backfill]** estiver ativado, todos os arquivos atuais no caminho especificado serão ingeridos durante a primeira ingestão programada. Se **[!UICONTROL Backfill]** estiver desativado, apenas os ficheiros carregados entre a primeira execução da ingestão e a **[!UICONTROL hora do Start]** serão assimilados. Os arquivos carregados antes de **[!UICONTROL hora do Start]** não serão assimilados. |
