...
>[!NOTE]
>
>O Adobe Experience Platform Launch está sendo reformulado como um conjunto de tecnologias de coleção de dados na Experience Platform. Como resultado, várias alterações de terminologia foram implementadas na documentação do produto. Consulte o seguinte [documento](../../../launch-term-updates.md) para obter uma referência consolidada das alterações de terminologia.

>[!IMPORTANT]
>
>A extensão deve ser usada com a variável [!DNL Adobe Experience Platform Web SDK] extensão. Para ver informações sobre a versão a ser usada com o AppMeasurement, consulte este link.

Use essa referência para obter informações sobre como configurar a extensão de plug-ins do SDK da Web e as opções disponíveis ao usar esta extensão para aumentar o [!DNL Adobe Experience Platform Web SDK] Extensão.
...
>[!IMPORTANT]
>
>A extensão de plug-ins comuns do SDK da Web tem como objetivo aumentar a variável [!DNL Adobe Experience Platform Web SDK] no entanto, não é necessário instalá-la para que a extensão funcione conforme esperado.

Nenhuma configuração adicional é necessária no nível da extensão.

## Adicionar plug-ins à extensão [!DNL Adobe Experience Platform Web SDK]

Diferentemente da contraparte do AppMeasurement, nenhuma configuração é necessária para inicializar ou adicionar um plug-in à biblioteca, além de usar os elementos de dados nativos fornecidos. Consulte abaixo informações adicionais sobre cada elemento de dados.

## Elementos de dados da extensão de plug-ins comuns do SDK da Web

A extensão de plug-ins comuns do SDK da Web fornece os seguintes elementos de dados:
...
Permite que os usuários aproveitem a interface nativa da coleta de dados no Adobe Experience Platform para configurar o [Plug-in getAndPersistValue](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getandpersistvalue.html). O `getAndPersistValue` o elemento de dados permite armazenar um valor em um cookie que pode ser recuperado posteriormente durante uma visita. Ele desempenha uma função semelhante ao recurso [!UICONTROL Duração de armazenamento] no Adobe Experience Platform Launch.
...
Permite que os usuários aproveitem a interface nativa da coleta de dados no Adobe Experience Platform para configurar o [plug-in getGeoCoordinates](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getgeocoordinates.html). O `getGeoCoordinates` o elemento de dados permite capturar a latitude e a longitude dos dispositivos dos visitantes.
...
Permite que os usuários aproveitem a interface nativa da coleta de dados para configurar o [Plug-in getNewRepeat](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getnewrepeat.html). O `getNewRepeat` o elemento de dados permite determinar se um visitante do site é um novo visitante ou um visitante repetido dentro de um número desejado de dias.
...
Permite que os usuários aproveitem a interface nativa da coleta de dados para configurar o [plug-in getPageName](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getpagename.html). O `getPageName` o elemento de dados cria uma versão formatada amigável e fácil de ler do URL atual.
...
Permite que os usuários aproveitem a interface nativa da coleta de dados para configurar o [plug-in getPreviousValue](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getpreviousvalue.html). O `getPreviousValue` o elemento de dados permite definir uma variável como um valor definido em uma ocorrência anterior.
...
Permite que os usuários aproveitem a interface nativa da coleta de dados para configurar o [plug-in getTimeParting](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/gettimeparting.html). O `getTimeParting` o elemento de dados permite capturar os detalhes do tempo em que qualquer atividade mensurável ocorre no site. Esse elemento de dados é importante quando você deseja detalhar métricas em qualquer divisão de tempo repetível em um determinado intervalo de datas. Por exemplo, você pode comparar as taxas de conversão de dois dias diferentes da semana, como todos os domingos e todas as quintas-feiras. Você também pode comparar períodos do dia, como todas as manhãs e todas as noites.
...
Permite que os usuários aproveitem a interface nativa da coleta de dados para configurar o [plug-in getTimeSinceLastVisit](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/gettimesincelastvisit.html). O `getTimeSinceLastVisit` o elemento de dados permite rastrear quanto tempo um visitante levou para retornar ao site após sua última visita.
...
Permite que os usuários aproveitem a interface nativa da coleta de dados para configurar o [plug-in getValOnce](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvalonce.html). O `getValOnce` o elemento de dados impede que uma variável seja definida com o mesmo valor mais de uma vez.
...
Permite que os usuários aproveitem a interface nativa da coleta de dados para configurar o [plugin getVisitDuration](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvisitduration.html). O `getVisitDuration` o elemento de dados rastreia o tempo em minutos que o visitante esteve no site até o momento.
