...
* **Um novo endpoint foi adicionado:** `{{extension_package_id}}/versions` permite que os usuários da API vejam todas as versões de pacotes de extensão que são públicas. Os usuários da API também podem ver informações de versão para pacotes de extensão que estão em desenvolvimento ou privados para organizações às quais pertencem. Consulte [https://experienceleague.adobe.com/docs/experience-platform/tags/api/endpoints/extension-packages.html](../api/endpoints/extension-packages.md#list-the-versions-for-an-extension-package)
