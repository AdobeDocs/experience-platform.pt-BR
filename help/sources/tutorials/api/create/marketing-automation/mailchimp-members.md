...
>[!TIP]
>
>Para recuperar o tipo de formato aceito para `{SOURCE_PARAMS}`, você deve codificar toda a cadeia de caracteres `list_id` em base64. Por exemplo, `"list_id": "10c097ca71"` codificado em base64 equivale a `eyJsaXN0SWQiOiIxMGMwOTdjYTcxIn0=`.
...
| `{SOURCE_PARAMS}` | Uma cadeia de caracteres codificada na base64 de `list_id`. |
...
| `params.listId` | Também conhecida como ID de público-alvo, a ID de lista [!DNL Mailchimp] permite a transferência de dados de público-alvo para outras integrações. |
