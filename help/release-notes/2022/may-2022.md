...
| --- | --- | --- |
| Grupo de campos | [[!UICONTROL Change set]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/changeset.schema.json) | Captura alterações no nível de linha de e para conjuntos de dados. Este grupo de campos pode ser empregado por qualquer classe. |
| Grupo de campos | [[!UICONTROL Reference keys]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/experienceevent-reference-keys.schema.json) | Captura chaves de referência para esquemas ExperienceEvent, permitindo construir relacionamentos com esquemas com base em outras classes. |
...
| --- | --- | --- |
| Comportamento | [[!UICONTROL Time-series Schema]](https://github.com/surbhi114/xdm/blob/master/components/behaviors/time-series.schema.json) | Atualização de `eventType` para incluir vários novos tipos de evento relacionados à mídia e um caso de uso de entrada de canal da Web para o Adobe Journey Optimizer. |
| Esquema global | [[!UICONTROL Destination]](https://github.com/tumulurik/xdm/blob/master/schemas/destinations/destination.schema.json) | Valores de enumeração removidos de `xdm:destinationCategory`. |
| Grupo de campos | [[!UICONTROL Record Status]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/record-status.schema.json) | Status do grupo de campos atualizado de `experimental` para `stable`. |
...
| Tipo de dados | [[!UICONTROL Browser details]](https://github.com/liljenback/xdm/blob/master/components/datatypes/browserdetails.schema.json) | Adição de um novo campo `xdm:userAgentClientHints` que captura informações contextuais sobre o agente do usuário que interage com o navegador. |
| Tipo de dados | [[!UICONTROL Media information]](https://github.com/lidiaist/xdm/blob/master/docs/reference/datatypes/media.schema.json) | Adição de um campo `xdm:playhead` para capturar o tempo do indicador de reprodução de um conteúdo de mídia. Correção da validação de padrão para `xdm:videoSegment`. |
| Tipo de dados | [[!UICONTROL Rating]](https://github.com/lidiaist/xdm/blob/master/components/datatypes/external/iptc/rating.schema.json) | `iptc4xmpExt:RatingSourceLink` não é mais um campo obrigatório. |
