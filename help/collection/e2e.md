...
>
>Todas as etapas que envolvem a interface da Coleção de dados também podem ser executadas na interface do Experience Platform.

## Pré-requisitos

Este tutorial usa a interface da Coleção de dados para criar um esquema, configurar um fluxo de dados e instalar o Web SDK. Para executar essas ações na interface do usuário, você deve ter acesso a pelo menos uma propriedade da Web, juntamente com os [direitos de propriedade](../tags/ui/administration/user-permissions.md#property-rights) a seguir:
...
Se você já tiver acesso ao Experience Platform, deverá garantir que todas as [permissões](../access-control/home.md#permissions) nas seguintes categorias estejam habilitadas:

* Modelagem de dados
* Identidades

Consulte a [visão geral da interface do controle de acesso](../access-control/ui/overview.md) para saber como conceder permissões para recursos do Experience Platform aos usuários.

## Resumo do processo

O processo de configuração da coleta de dados para seu site pode ser resumido da seguinte maneira:

1. [Crie um esquema](#schema) para determinar como seus dados serão estruturados ao serem enviados para a Edge Network.
1. [Crie uma sequência de dados](#datastream) para configurar para quais destinos você deseja que seus dados sejam enviados.
1. [Instale e configure o Web SDK](#sdk) para enviar dados para a sequência de dados quando determinados eventos ocorrerem no site.

Depois de enviar dados para a Edge Network, você também poderá [configurar o encaminhamento de eventos](#event-forwarding), se sua organização tiver uma licença para isso.
...
>[!NOTE]
>
>Os esquemas XDM são muito personalizáveis. Em vez de serem excessivamente prescritivas, as etapas descritas abaixo se concentram especificamente nos requisitos de esquema do Web SDK. Fora desses parâmetros, você pode definir a estrutura restante dos dados da maneira que desejar.
...
Uma caixa de diálogo é exibida solicitando que você comece a adicionar grupos de campos ao esquema. Para enviar eventos usando o Web SDK, é necessário adicionar o grupo de campos **[!UICONTROL AEP Web SDK ExperienceEvent Mixin]**. Este grupo de campos contém definições para atributos de dados que são coletados automaticamente pela biblioteca do Web SDK.
...
>
>A prática recomendada é adicionar campos somente para dados que você planeja enviar por meio da Edge Network. Depois de adicionar campos a um esquema e salvá-lo, somente alterações aditivas poderão ser feitas no esquema após isso. Consulte a seção sobre as [regras de evolução do esquema](../xdm/schema/composition.md#evolution) para obter mais informações.
...
>[!NOTE]
>
>Se você quiser usar o [encaminhamento de eventos](../tags/ui/event-forwarding/overview.md) (supondo que sua organização esteja licenciada para a funcionalidade), será necessário habilitá-lo para uma sequência de dados da mesma forma que habilita os produtos da Adobe. Detalhes sobre este processo são abordados em uma [seção posterior](#event-forwarding).
...
## Instalar e configurar o Web SDK {#install}
...
>[!NOTE]
>
>Esta seção usa a interface da Coleção de dados para configurar a extensão de tag do Web SDK, mas você também pode instalá-la e configurá-la usando código bruto. Consulte os seguintes guias para obter mais informações:
>
>* [Instalar o SDK](/help/web-sdk/install/overview.md)
>* [Configurar o SDK](/help/web-sdk/commands/configure/overview.md)
>
>Observe também que mesmo que você deseje usar apenas o encaminhamento de eventos, ainda será necessário instalar e configurar o SDK conforme descrito antes de configurar o encaminhamento de eventos em uma [etapa posterior](#event-forwarding).

O processo pode ser resumido da seguinte forma:

1. [Instale o Adobe Experience Platform Web SDK em uma propriedade de marca](#install-sdk) para obter acesso a seus recursos.
...
1. [Crie uma regra](#rule) para informar à SDK quando ela deve enviar dados para a Edge Network.
1. [Crie e instale uma biblioteca](#library) para implementar a regra no seu site.

### Instalar a SDK em uma propriedade de tag {#install-sdk}
...
A página de visão geral da propriedade é exibida. Aqui, selecione **[!UICONTROL Extensões]** na navegação à esquerda e **[!UICONTROL Catálogo]**. Localize a listagem do Experience Platform Web SDK (opcionalmente, use a barra de pesquisa para restringir os resultados) e selecione **[!UICONTROL Instalar]**.

![Instalar o Web SDK](./images/e2e/install-sdk.png)
...
![Configurar o Web SDK](./images/e2e/configure-sdk.png)
...
>[!NOTE]
>
>Para obter mais informações sobre os diferentes tipos de eventos fornecidos pelas extensões da Web do Adobe, incluindo como configurá-los, consulte a [referência das extensões da Adobe](../tags/extensions/client/overview.md) na documentação das tags.
...
A página de configuração da ação é exibida. Para obter a regra para enviar dados à Edge Network, selecione **[!UICONTROL Adobe Experience Platform Web SDK]** para a extensão e **[!UICONTROL Enviar evento]** para o tipo de ação.
...
Depois de instalar o ambiente em seu site, você pode [testar sua implementação](../tags/ui/publishing/embed-code-testing.md) usando o Adobe Experience Platform Debugger.
