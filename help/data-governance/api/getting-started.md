...
`core` os recursos são os definidos e mantidos por Adobe, enquanto `custom` os recursos são aqueles criados e mantidos pela sua organização, e, portanto, são exclusivos e visíveis exclusivamente na sua organização. Assim, operações de listagem e pesquisa (`GET`) são as únicas operações permitidas em `core` recursos, enquanto as operações de listagem, pesquisa e atualização (`POST`, `PUT`, `PATCH`e `DELETE`) estão disponíveis para `custom` recursos.
