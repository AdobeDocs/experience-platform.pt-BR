...
# Crie uma conexão de origem e um fluxo de dados para [!DNL Pinterest Ads] usando a API [!DNL Flow Service]
...
O tutorial a seguir guiará você pelas etapas para criar uma conexão de origem e um fluxo de dados do [!DNL Pinterest Ads] para trazer dados do [[!DNL Pinterest Ads]](https://ads.pinterest.com/) para a Adobe Experience Platform usando a [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
As seções a seguir fornecem informações adicionais que você precisará saber para se conectar com êxito ao [!DNL Pinterest Ads] usando a API [!DNL Flow Service].
...
* O [!DNL Pinterest] `accessToken`.
* O [!DNL Pinterest] `adAccountId`.
* Uma das IDs de [!DNL Pinterest] `campaign`, `adGroup` ou `ad`, conforme necessário.
...
## Conectar o [!DNL Pinterest Ads] ao Experience Platform usando a API [!DNL Flow Service] {#connect-platform-to-flow-api}

Veja a seguir as etapas a serem seguidas para conectar o [!DNL Pinterest Ads] ao Experience Platform.

### Criar uma conexão básica {#base-connection}

Uma conexão base retém informações entre sua origem e a Experience Platform, incluindo as credenciais de autenticação da origem, o estado atual da conexão e a ID de conexão base exclusiva. A ID de conexão básica permite explorar e navegar pelos arquivos de dentro da origem e identificar os itens específicos que deseja assimilar, incluindo informações sobre os tipos de dados e formatos.

Para criar uma ID de conexão base, faça uma solicitação POST para o ponto de extremidade `/connections` ao fornecer suas credenciais de autenticação do [!DNL Pinterest Ads] como parte do corpo da solicitação.
...
| `auth.params.accessToken` | Contém o valor do Token de Acesso [!DNL Pinterest] necessário para autenticar sua origem. |
...
Use as chamadas a seguir para localizar o caminho do arquivo que deseja trazer para o Experience Platform:
...
| `{SOURCE_PARAMS}` | Define parâmetros para o arquivo de origem que você deseja trazer para a Experience Platform. Para recuperar o tipo de formato aceito para `{SOURCE_PARAMS}`, você deve codificar toda a cadeia de caracteres `{"ad_account_id":"{PINTEREST_AD_ACCOUNT_ID}","object_ids":"{COMMA_SEPERATED_OBJECT_IDS}","object_type":"{OBJECT_TYPE}}"}` em base64. |

[!DNL Pinterest Ads] dá suporte a vários pontos de extremidade de API do Analytics [!DNL Pinterest]. Dependendo do tipo de objeto que você está usando, a solicitação a ser enviada é como a seguir:
...
Para [!DNL Pinterest Ads], ao aproveitar a API do Campaign Analytics, o valor para `{SOURCE_PARAMS}` é passado como `{"ad_account_id":"123456789000","object_ids":"000123456789","object_type":"campaigns"}`. Quando codificado em base64, ele equivale a `YHsiYWRfYWNjb3VudF9pZCI6IjEyMzQ1Njc4OTAwMCIsIm9iamVjdF9pZHMiOiIwMDAxMjM0NTY3ODkiLCJvYmplY3RfdHlwZSI6ImNhbXBhaWducyJ9`, como mostrado abaixo.
...
Para [!DNL Pinterest Ads], ao aproveitar a API do Analytics de grupos de anúncios, o valor para `{SOURCE_PARAMS}` é passado como `{"ad_account_id":"123456789000","object_ids":"000123456789,100123456789","object_type":"ad_groups"}`. Quando codificado em base64, ele equivale a `eyJhZF9hY2NvdW50X2lkIjoiMTIzNDU2Nzg5MDAwIiwib2JqZWN0X2lkcyI6IjAwMDEyMzQ1Njc4OSwxMDAxMjM0NTY3ODkiLCJvYmplY3RfdHlwZSI6ImFkX2dyb3VwcyJ9`, como mostrado abaixo.
...
Uma resposta bem-sucedida retorna a estrutura de dados da API [!DNL Pinterest Ads] correspondente chamada.
...
Você pode criar uma conexão de origem fazendo uma solicitação POST para a API [!DNL Flow Service]. Uma conexão de origem consiste em uma ID de conexão de base, um caminho para o arquivo de dados de origem e uma ID de especificação de conexão.
...
A origem [!DNL Pinterest Ads] dá suporte a vários pontos de extremidade de API do Analytics [!DNL Pinterest]. Dependendo do tipo de objeto que você estiver utilizando, a solicitação a seguir criará uma conexão de origem:
...
| `data.format` | O formato dos dados [!DNL Pinterest Ads] que você deseja assimilar. Atualmente, o único formato de dados com suporte é `json`. |
| `params.ad_account_id` | O [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | Como o ponto de extremidade da API do Campaign Analytics [!DNL Pinterest] é obrigatório, o valor será `campaigns`. |
...
| `data.format` | O formato dos dados [!DNL Pinterest Ads] que você deseja assimilar. Atualmente, o único formato de dados com suporte é `json`. |
| `params.ad_account_id` | O [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | Como o ponto de extremidade da API do Analytics para grupos de anúncios [!DNL Pinterest] é obrigatório, o valor será `ad_groups`. |
...
| `data.format` | O formato dos dados [!DNL Pinterest Ads] que você deseja assimilar. Atualmente, o único formato de dados com suporte é `json`. |
| `params.ad_account_id` | O [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | Como o ponto de extremidade da API do Ad Analytics [!DNL Pinterest] é obrigatório, o valor será `ads`. |
...
| `data.format` | O formato dos dados do [!DNL Pinterest Ads] que você deseja trazer para o Experience Platform. |
...
A última etapa para trazer dados de [!DNL Pinterest Ads] para a Experience Platform é criar um fluxo de dados. Até agora, você tem os seguintes valores necessários preparados:
