...
Carregue o `gtag.js` do Google em seu site para enviar dados de evento para [!DNL Google Analytics], Google Ads e [!DNL Google Marketing Platform]. Essa extensão adiciona somente o código da tag ao site. Você precisará usar outras extensões do Google para adicionar eventos e ações que usarão gtag.
...
Para obter informações sobre as opções de configuração de extensão e o suporte de instalação, consulte a página [Google gtag no Adobe Exchange](https://exchange.adobe.com/experiencecloud.details.102805.google-gtag.html).
