...
title: Criar uma conexão de origem HP Vertica usando a API de serviço de fluxo
topic: overview
type: Tutorial
description: Saiba como conectar o HP Vertica à Adobe Experience Platform usando a API de Serviço de Fluxo.
...
Este tutorial usa a API [!DNL Flow Service] para guiá-lo pelas etapas para conectar HP [!DNL Vertica] a [!DNL Experience Platform].
...
Para que [!DNL Flow Service] se conecte com o HP [!DNL Vertica], é necessário fornecer valores para as seguintes propriedades de conexão:
...
| `connectionSpec.id` | A ID de especificação de conexão HP [!DNL Vertica]: `a8b6a1a4-5735-42b4-952c-85dce0ac38b5`. |
...
Ao seguir este tutorial, você criou uma conexão HP [!DNL Vertica] usando a API [!DNL Flow Service] e obteve o valor de ID exclusivo da conexão. Você pode usar essa ID no próximo tutorial à medida que aprende a [explorar bancos de dados usando a API de Serviço de Fluxo](../../explore/database-nosql.md).
