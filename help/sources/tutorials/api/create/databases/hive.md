...
# Criar um Apache Hive no conector HDInsights do Azure usando a API de Serviço de Fluxo
...
Este tutorial usa a API de Serviço de Fluxo para guiá-lo pelas etapas para conectar o Apache Hive no Azure HDInsights ao Experience Platform.
...
As seções a seguir fornecem informações adicionais que você precisará saber para se conectar ao Hive com êxito usando a API de Serviço de Fluxo.

### Reunir credenciais obrigatórias

Para que o Serviço de Fluxo se conecte ao Hive, é necessário fornecer valores para as seguintes propriedades de conexão:

| Credencial | Descrição |
| ---------- | ----------- |
| `host` | Endereço IP ou nome do host do servidor de Coleção. |
| `username` | O nome de usuário que você usa para acessar o servidor do Hive. |
| `password` | A senha correspondente ao usuário. |
| `connectionSpec.id` | O identificador exclusivo necessário para criar uma conexão. A ID de especificação de conexão para Hive é: `aac9bbd4-6c01-46ce-b47e-51c6f0f6db3f` |
...
Para criar uma conexão Hive, a ID de especificação de conexão exclusiva deve ser fornecida como parte da solicitação POST. A ID de especificação de conexão para Hive é `aac9bbd4-6c01-46ce-b47e-51c6f0f6db3f`.
...
| Parâmetro | Descrição |
| --------- | ----------- |
| `auth.params.connectionString` | A cadeia de conexão associada à sua conta Hive. |
| `connectionSpec.id` | A ID da especificação da conexão Hive: `aac9bbd4-6c01-46ce-b47e-51c6f0f6db3f`. |
...
Ao seguir este tutorial, você criou uma conexão Hive usando a API de Serviço de Fluxo e obteve o valor de ID exclusivo da conexão. Você pode usar essa ID no próximo tutorial à medida que aprende a [explorar bancos de dados usando a API](../../explore/database-nosql.md)do Serviço de Fluxo.
