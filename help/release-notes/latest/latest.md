...
| Recurso | Descrição |
| --- | --- |
| Gerenciamento de inventário do conjunto de dados | A interface do usuário de conjuntos de dados agora oferece uma coleção de ações em linha para gerenciar melhor seus conjuntos de dados. O gerenciamento avançado de conjuntos de dados melhora a eficiência do seu trabalho por meio da criação e atribuição de pastas e tags aos seus conjuntos de dados, o que permite filtrar e melhorar a capacidade de descoberta. Consulte a documentação para obter mais informações sobre [ações embutidas](../../catalog/datasets/user-guide.md#inline-actions), como [pesquisar e filtrar conjuntos de dados](../../catalog/datasets/user-guide.md#search-and-filter), e [mover conjuntos de dados para pastas](../../catalog/datasets/user-guide.md#move-to-folders). |

{style="table-layout:auto"}

Para obter mais informações sobre o Serviço de Catálogo, consulte [Visão geral do Serviço de catálogo](../../catalog/home.md).
...
| Recurso | Descrição |
| --- | --- |
| Alternância do Editor de Consulta Aprimorado | O botão aprimorado Editor de consultas oferece melhor acessibilidade e suporte a vários temas. As configurações aprimoradas do editor permitem ativar temas escuros ou claros. Consulte a [documentação da](../../query-service/ui/user-guide.md#enhanced-editor-toggle) para obter mais informações. |
| Nome de alias para estatísticas calculadas | Agora você pode fornecer um nome de alias para fazer referência descritiva aos resultados de suas estatísticas calculadas em consultas SQL. Consulte a documentação para obter informações sobre esta e outras atualizações do comando COMPUTE STATISTICS. Consulte a [documentação da](../../query-service/essential-concepts/dataset-statistics.md#alias-name) para obter mais informações. |
...
| Grupo de campos | [[!UICONTROL Detalhes enriquecidos do segmento de evento]](https://github.com/adobe/xdm/pull/1754/files) | Uma lista de públicos para os quais o perfil se qualifica no momento da coleta do evento. |
...
| --- | --- | --- |
| Grupo de campos | [[!UICONTROL Detalhes de interação do MediaAnalytics]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de experimental para `stable`. |
| Grupo de campos | [[!UICONTROL Detalhes de interação com a mídia]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `stable` para `deprecated`. |
| Tipo de dados | [[!UICONTROL Informações de detalhes da sessão]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Informações detalhadas de dados de QoE]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Informações de dados do estado do player]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental`para `stable`. |
| Tipo de dados | [[!UICONTROL Informações do evento de mídia]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Informações de detalhes de mídia]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Informações de detalhes do erro]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Informações de detalhes do erro]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `stable` para `deprecated`. |
| Tipo de dados | [[!UICONTROL Informações detalhadas de metadados personalizados]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Informações detalhadas do capítulo]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Informações detalhadas do Pod de publicidade]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Informações de detalhes de publicidade]](https://github.com/adobe/xdm/pull/1756/files) | A variável `meta:status` foi atualizado de `experimental` para `stable`. |
| Extensão (Gerenciamento de Jornada do cliente) | [[!UICONTROL Domínio]](https://github.com/adobe/xdm/pull/1756/files) | `Domain` campo adicionado a [!UICONTROL Adobe CJM ExperienceEvent - Detalhes do perfil da mensagem] para registrar o domínio do endereço de email do recipient. |
| Extensão (Gerenciamento de Jornada do cliente) | [[!UICONTROL Nome da variante do canal]](https://github.com/adobe/xdm/pull/1753/files) | Este campo foi adicionado a [!UICONTROL Campos de entidade do AJO] para representar o nome da variante de canal. |
