---
title: Criar uma conexão e um fluxo de dados do Source para Pendo usando a API do serviço de fluxo
...
# Crie uma conexão de origem e um fluxo de dados para [!DNL Pendo] usando a API de Serviço de Fluxo
...
O tutorial a seguir guiará você pelas etapas para criar uma conexão de origem e um fluxo de dados para trazer dados do evento [[!DNL Pendo]](https://Pendo.com/) para a Adobe Experience Platform usando a [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| `data.format` | O formato dos dados [!DNL Pendo] que você deseja assimilar. Atualmente, o único formato de dados com suporte é `json`. |
...
| `data.format` | O formato dos dados [!DNL Pendo] que você deseja assimilar. |
...
Uma resposta bem-sucedida retorna informações sobre seu fluxo de dados, incluindo a URL do ponto de extremidade, marcado como `inletUrl`. Consulte a página [Webhook de instalação](../../../ui/create/analytics/pendo-webhook.md#get-streaming-endpoint-url) para obter o valor necessário.
