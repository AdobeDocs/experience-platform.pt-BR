...
| -------- | ----------- |
| `connectionId` | A ID de conexão do aplicativo de protocolos |
...
Um dataflow é responsável por coletar dados de fontes e trazê-los para a Plataforma. Para criar um fluxo de dados, primeiro você deve obter as especificações de fluxo de dados responsáveis pela coleta de dados do aplicativo de protocolos.
...
| `transformations.params.deltaColum` | A coluna designada usada para diferenciar entre dados novos e existentes. Os dados incrementais serão ingeridos com base no carimbo de data e hora da coluna selecionada. O formato suportado para `deltaColumn` ao usar OData Genérico é `yyyy-MM-ddTHH:mm:ssZ`. |
