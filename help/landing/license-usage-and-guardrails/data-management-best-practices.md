...
O Experience Platform é composto principalmente por dois repositórios de dados: o [!DNL data lake] e a Loja de perfis.
...
* A soma de todos os dados de produção armazenados no Adobe Real-time Customer Data Platform (ou seja, Perfil do cliente em tempo real e Serviço de identidade) a qualquer momento, dividida pelo [!DNL Addressable Audience];
* A soma de todos os dados armazenados na Platform (incluindo, mas não limitado à [!DNL data lake], Perfil do cliente em tempo real e Serviço de identidade) a qualquer momento e quaisquer dados transmitidos (em vez de armazenados na) plataforma nos últimos 12 meses, divididos pelo [!DNL Addressable Audience].
...
Os dados podem ser assimilados em um ou vários sistemas na Platform, ou seja, o [!DNL data lake] e/ou a Loja de perfis. Isso significa que dados diferentes podem existir em ambos os sistemas para uma variedade de casos de uso diferentes. Por exemplo, talvez você queira manter os dados históricos na variável [!DNL data lake], mas não na loja de Perfis. Você pode selecionar quais dados enviar para o armazenamento de Perfil ativando um conjunto de dados para assimilação de Perfil.
...
| --- | --- |
| Filtragem de origem do Adobe Audience Manager | Ao criar uma conexão de origem do Adobe Audience Manager, você pode escolher quais segmentos e características trazer para a [!DNL data lake] e o Perfil do cliente em tempo real, em vez de assimilar os dados de Audience Manager na íntegra. Consulte o guia sobre [criação de uma conexão de origem de Audience Manager](../../sources/tutorials/ui/create/adobe-applications/audience-manager.md) para obter mais informações. |
...
| Exclusão de dados do encaminhamento pelo lado do servidor | Se estiver enviando dados para a Platform usando o encaminhamento pelo lado do servidor, você poderá excluir quais dados são enviados removendo o mapeamento em uma ação de regra para excluí-los em todos os eventos ou adicionando condições à regra para que os dados só sejam acionados para determinados eventos. Consulte a documentação em [eventos e condições](/help/tags/ui/managing-resources/rules.md#events-and-conditions-if) para obter mais informações. |
...
* Configurar [Expirações do evento de experiência](#event-expirations) e [Expirações de dados do perfil de pseudônimo](#pseudonymous-profile-expirations) para dados de alta frequência, como dados da Web.
* Verifique periodicamente a [Relatórios de composição de perfil](#profile-store-composition-reports) para entender a composição da loja de perfis. Isso permite compreender as fontes de dados que mais contribuem para o consumo de licença.
...
| Recurso | Descrição |
| --- | --- |
| [Ativar/desativar conjuntos de dados para perfil](../../catalog/datasets/user-guide.md) | Ative ou desative a assimilação do conjunto de dados no Perfil do cliente em tempo real. |
