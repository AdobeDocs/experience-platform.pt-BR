...
Esta página descreve como usar o Destination SDK para configurar um destino Amazon S3 com personalização [opções de formatação de arquivo](configure-file-formatting-options.md) e um [configuração do nome do arquivo](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).
