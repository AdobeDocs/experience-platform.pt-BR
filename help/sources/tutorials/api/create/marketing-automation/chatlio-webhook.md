...
# Criar uma conexão de origem e um fluxo de dados para [!DNL Chatlio] uso da API do Serviço de fluxo
...
O tutorial a seguir orienta você pelas etapas para criar uma conexão de origem e um fluxo de dados para trazer [[!DNL Chatlio]](https://chatlio.com/) dados do evento para a Adobe Experience Platform usando o [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Conectar [!DNL Chatlio] para a Platform usando o [!DNL Flow Service] API {#connect-platform-to-flow-api}

A seguir, há uma descrição das etapas necessárias para criar uma conexão de origem e um fluxo de dados para trazer seus [!DNL Chatlio] dados de eventos para o Experience Platform.
...
| `data.format` | O formato do [!DNL Chatlio] dados que você deseja assimilar. No momento, o único formato de dados compatível é `json`. |
...
O último passo para trazer dados de [!DNL Chatlio] para a Platform é criar um fluxo de dados. Até agora, você tem os seguintes valores necessários preparados:
...
Uma resposta bem-sucedida retorna informações sobre o fluxo de dados, incluindo a URL do ponto de extremidade, marcado como `inletUrl`. Consulte a [Configurar Webhook](../../../ui/create/marketing-automation/chatlio-webhook.md#get-streaming-endpoint-url) para obter o valor necessário.
