...
Você pode recuperar uma lista de todos os destinos enviados para publicação da sua organização, fazendo uma solicitação de GET para a `/authoring/destinations/publish` endpoint .
...
A solicitação a seguir recupera a lista de destinos enviados para publicação aos quais você tem acesso, com base na organização e na configuração da sandbox.
...
A resposta a seguir retorna o status HTTP 200 com uma lista de destinos enviados para publicação aos quais você tem acesso, com base na ID da organização e no nome da sandbox usados. One `configId` corresponde à solicitação de publicação para um destino.
