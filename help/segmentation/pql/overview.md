...
[!DNL Profile Query Language] (PQL) é um [!DNL Experience Data Model] Linguagem de consulta compatível com o XDM, projetada para oferecer suporte à definição e execução de consultas de segmentação para [!DNL Real-Time Customer Profile] dados.
...
No exemplo abaixo, como o primeiro parâmetro está sempre no contexto, uma referência de propriedade (`homeAddress`) pode ser feito diretamente a ela.
...
No exemplo abaixo, `$1` refere-se ao primeiro parâmetro. Como resultado, `$2` refere-se ao segundo parâmetro etc.
...
| Categoria | Definição |
| -------- | ---------- |
| Booleano | Usada para implementar álgebra booleana no PQL. Mais informações sobre essas funções podem ser encontradas no [documento de funções booleanas](./boolean-functions.md). |
...
| Matriz, lista e conjunto | Usado para interagir com arrays, listas e conjuntos. Mais informações sobre essas funções podem ser encontradas no [documento de funções de matriz, lista e definição](./array-functions.md). |
| Mapa | Usado para interagir com mapas. Mais informações sobre essas funções podem ser encontradas no [documento de funções do mapa](./map-functions.md). |
...
| Data e hora | Usada em conjunto com objetos date, time e datetime. Mais informações sobre essas funções podem ser encontradas no [documento de funções de data/hora](./datetime-functions.md). |
| Filtro | Usado para filtrar dados em arrays. Mais informações sobre essas funções podem ser encontradas no [filtrar documento de funções](./filter-functions.md). |
| quantificadores lógicos | Usado para asserção de condições em um storage. Mais informações podem ser encontradas no [documento quantificadores lógicos](./logical-quantifiers.md). |
...
Agora que você aprendeu a usar [!DNL Profile Query Language], você pode usar PQL ao criar e modificar segmentos. Para obter mais informações sobre a segmentação, leia o [visão geral da segmentação](../home.md).
