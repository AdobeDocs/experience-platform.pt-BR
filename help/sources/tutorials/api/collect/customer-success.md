...
Este tutorial aborda as etapas para recuperar dados de um sistema bem-sucedido do cliente e assimilá-los na Plataforma por meio de conectores de origem e APIs.

## Introdução

Este tutorial requer que você tenha acesso a um sistema bem-sucedido de terceiros por meio de uma conexão válida e informações sobre o arquivo que deseja trazer para a Plataforma, incluindo o caminho e a estrutura do arquivo. Se você não tiver essas informações, consulte o tutorial sobre como [explorar um banco de dados ou um sistema NoSQL usando a API](../explore/customer-success.md) do Serviço de Fluxo antes de tentar este tutorial.
...
As seções a seguir fornecem informações adicionais que você precisará saber para se conectar com êxito a um sistema bem-sucedido do cliente usando a API de Serviço de Fluxo.
...
| `connectionSpec.id` | A ID de especificação de conexão associada ao seu sistema de sucesso de terceiros específico. Consulte o [apêndice](#appendix) para obter uma lista de IDs de especificação de conexão. |
...
Uma resposta bem-sucedida retorna os detalhes da especificação de fluxo de dados responsável por trazer os dados do sistema bem-sucedido do cliente para a Plataforma. Essa ID é necessária na próxima etapa para criar um novo fluxo de dados.
