...
A configuração de consentimento padrão para [!DNL Segment Match] está definida como `opt-out`. Para aplicar um modelo de aceitação para seus dados, envie uma solicitação por email para o Gerente de conta do Adobe.
...
| Permissão | Descrição |
| --- | --- |
| Gerenciar conexões de compartilhamento de público | Essa permissão permite concluir o processo de handshake de parceiro, que conecta duas Organizações IMS para habilitar [!DNL Segment Match] fluxos. |
...
>title="Casos de uso de marketing restrito"
>abstract="Casos de uso de marketing restrito ajudam a fornecer orientação aos seus parceiros para garantir que os segmentos compartilhados sejam usados corretamente de acordo com as restrições de governança de dados."
