...
# Colete dados de protocolos usando conectores de origem e APIs

Este tutorial aborda as etapas para recuperar dados de um aplicativo de protocolo de terceiros e assimilá-los no Adobe Experience Platform por meio de conectores de origem e da [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

## Introdução

Este tutorial requer que você tenha acesso a um sistema de protocolo por meio de uma conexão base válida e informações sobre o arquivo que deseja trazer para a Platform, incluindo o caminho e a estrutura da tabela. Se não tiver essas informações, consulte o tutorial em [exploração de sistemas de protocolo usando a API do Serviço de Fluxo](../explore/protocols.md) antes de tentar este tutorial.
...
| -------- | ----------- |
| `baseConnectionId` | A ID de conexão do seu aplicativo de protocolos |
...
Um fluxo de dados é responsável por coletar dados de fontes e trazê-los para a plataforma. Para criar um fluxo de dados, primeiro você deve obter as especificações do fluxo de dados responsáveis pela coleta de dados do aplicativo de protocolos.
...
| `transformations.params.deltaColum` | A coluna designada usada para diferenciar dados novos e existentes. Os dados incrementais serão assimilados com base no carimbo de data e hora da coluna selecionada. O formato compatível para `deltaColumn` ao usar OData Genérico é `yyyy-MM-ddTHH:mm:ssZ`. |
