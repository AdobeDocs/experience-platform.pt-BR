...
O esquema [!DNL Bombora] é chamado de **Intenção da Conta Semanalmente**. São as informações de intenção semanal (pesquisa de comprador B2B anônimo e consumo de conteúdo) sobre contas e tópicos especificados. Os dados estão em formato parquet.
...
| `Topic_Id` | STRING | TRUE | SIM | A ID do tópico [!DNL Bombora]. |
| `Topic_Name` | STRING | TRUE | | O nome do tópico [!DNL Bombora]. |
| `Cluster_Name` | STRING | TRUE | | O nome do cluster em [!DNL Bombora] para um determinado tópico. |
...
| `Partition_Date` | DATA | TRUE | | A data do calendário de um instantâneo. Isso é feito semanalmente, no final da semana, no formato `mm/dd/yyyy`. |
