...
O Experience Platform fornece vários namespaces de identidade que estão disponíveis para todas as organizações. Eles são conhecidos como namespaces padrão e são visíveis usando a API [!DNL Identity Service] ou por meio da interface do usuário da plataforma.
...
| Adobe Analytics (ID herdada) | Um namespace que representa Adobe Analytics. Consulte o seguinte documento em [Adobe Analytics namespaces](https://experienceleague.adobe.com/docs/analytics/admin/data-governance/gdpr-namespaces.html?lang=en#namespaces) para obter mais informações. |
