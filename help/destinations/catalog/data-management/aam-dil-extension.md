...
Esta é a extensão do Adobe Audience Manager Data Integration Library (implementação do lado do cliente). Observação: esta extensão não deve ser usada para o encaminhamento pelo lado do servidor (SSF) de dados [!DNL Adobe Analytics]. Para SSF, use a extensão [!DNL Adobe Analytics]. Importante: a partir da versão 8.0, o DIL tem uma dependência forte no [!DNL Experience Cloud] ID Service, versão 3.3 ou superior. Implemente o Serviço de ID do [!DNL Experience Cloud] e o DIL para obter recursos completos de integração de dados do [!DNL Audience Manager].

[!DNL Audience Manager] O DIL é uma extensão da Plataforma de Gerenciamento de Dados (DMP) em [!DNL Adobe Experience Platform]. Para obter mais informações sobre a funcionalidade de extensão, consulte a [página de extensão do Audience Manager](../../../tags/extensions/client/audience-manager/overview.md) na documentação de tags.
