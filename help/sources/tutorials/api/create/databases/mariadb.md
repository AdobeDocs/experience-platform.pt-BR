...
title: Criar uma conexão de origem MariaDB usando a API de serviço de fluxo
topic: overview
type: Tutorial
description: Saiba como conectar o Adobe Experience Platform à MariaDB usando a API de Serviço de Fluxo.
...
# Criar uma conexão de origem [!DNL MariaDB] usando a API [!DNL Flow Service]
...
Este tutorial usa a API [!DNL Flow Service] para guiá-lo pelas etapas para conectar [!DNL Experience Platform] a [!DNL MariaDB].
...
Para que [!DNL Flow Service] se conecte com [!DNL MariaDB], é necessário fornecer a seguinte propriedade de conexão:

| Credencial | Descrição |
| ---------- | ----------- |
| `connectionString` | A cadeia de conexão associada à autenticação [!DNL MariaDB]. O padrão da cadeia de conexão [!DNL MariaDB] é: `Server={HOST};Port={PORT};Database={DATABASE};UID={USERNAME};PWD={PASSWORD}`. |
...
Para obter mais informações sobre como obter uma cadeia de conexão, consulte [este documento MariaDB](https://mariadb.com/kb/en/about-mariadb-connector-odbc/).
...
Para criar uma conexão [!DNL MariaDB], sua ID exclusiva de especificação de conexão deve ser fornecida como parte da solicitação de POST. A ID de especificação de conexão para [!DNL MariaDB] é `3000eb99-cd47-43f3-827c-43caf170f015`.
...
| -------- | ----------- |
| `auth.params.connectionString` | A cadeia de conexão associada à autenticação [!DNL MariaDB]. O padrão da cadeia de conexão [!DNL MariaDB] é: `Server={HOST};Port={PORT};Database={DATABASE};UID={USERNAME};PWD={PASSWORD}`. |
| `connectionSpec.id` | A ID de especificação de conexão [!DNL MariaDB] é: `3000eb99-cd47-43f3-827c-43caf170f015`. |
