...
* **Frequência**: configure a frequência para indicar a frequência de execução do fluxo de dados. Você pode definir a frequência como: uma vez, minuto, hora, dia ou semana.
* **Intervalo**: depois de selecionar uma frequência, você pode definir a configuração de intervalo para estabelecer o período entre cada assimilação. Por exemplo, se você definir a frequência como dia e configurar o intervalo como 15, o fluxo de dados será executado a cada 15 dias. O intervalo não pode ser definido como zero e deve ser definido como pelo menos 15.
* **Hora de Início** - O carimbo de data/hora da execução projetada, apresentado no fuso horário UTC.
