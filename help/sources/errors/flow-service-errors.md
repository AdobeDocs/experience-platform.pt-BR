...
| `1103-503` | Serviço indisponível | Serviço temporariamente indisponível. Tente novamente. Se o problema persistir, entre em contato com o suporte ao cliente. |
...
| `1401-400` | Solicitação inválida | Os parâmetros limite e contagem não podem ser fornecidos juntos na mesma solicitação. Forneça apenas o limite ou o parâmetro de contagem e tente novamente. |
...
| `1407-400` | Solicitação inválida | O status de falha não pode ser usado com o `progress` subOp. Remova o `progress` subOp ou use o status=&#39;inProgress&#39; e tente novamente. |
...
| `1416-400` | Solicitação inválida | Excluir ou Atualizar só pode ser aplicado na conexão com o estado habilitado, desabilitado ou inicializando. |
| `1417-400` | Solicitação inválida | Excluir ou atualizar em conexões no `initializing` estado não são permitidos com UserToken. |
...
| `1422-400` | Solicitação inválida | A ação desabilitar não pode ser aplicada no estado atual {state}. Atualize a ação e tente novamente. |
...
| `1424-400` | Solicitação inválida | OrderBy não é compatível com a consulta de sandbox cruzada. |
...
| `1426-400` | Solicitação inválida | O token do usuário não está autorizado a criar/atualizar a especificação de conexão. Verifique se o token do usuário está autorizado e tente novamente. |
| `1427-400` | Solicitação inválida | O token do usuário não está autorizado a criar/atualizar execuções de fluxo. Verifique se o token do usuário está autorizado e tente novamente. |
...
| `1431-400` | Solicitação inválida | &#39;Campos&#39; de matriz vazia não são permitidos na política. |
| `1432-400` | Solicitação inválida | A ordem de classificação especificada não está correta, inclua + como ordem crescente e - como ordem decrescente em fieldName. |
...
| `1445-400` | Solicitação inválida | Recurso de conexão de PATCH não habilitado. |
...
| `1454-400` | Solicitação inválida | Tipo de Valor de Filtro não tratado fornecido. |
| `1455-400` | Solicitação inválida | Ocorreu um erro ao validar as instruções do patch: Campo ausente &quot;keyWhichDoesNotExist&quot;. |
| `1456-400` | Solicitação inválida | Estado de exclusão-finalização não é um valor válido. Os valores permitidos são [ativado, desativado, inicializando]. |
...
| Código de erro | Título | Mensagem detalhada |
| --- | --- | --- |
| `2000-401` | Token de autorização inválido | O token de autorização não tem acesso a esta organização ou a organização não existe. Verifique se a organização existe ou entre em contato com o administrador para obter acesso. |
...
| `2102-500` | Obter sandbox por erro de nome | Ocorreu um problema ao recuperar uma sandbox com o nome &quot;dev&quot;. Erro da API de gerenciamento de sandbox: Algo deu errado. Tente novamente. |
...
| `2104-500` | Erro ao obter sandbox por identificador | Houve um problema ao recuperar uma sandbox com a id &#39;8da3ef09-b469-404a&#39; e o nome &#39;dev&#39;. Erro da API de gerenciamento de sandbox: Algo deu errado. Tente novamente. |
...
| `2106-404` | Obter erro de sandbox padrão | Não foi possível encontrar as informações padrão da sandbox. |
| `2107-500` | Obter erro de sandbox padrão | Ocorreu um problema ao recuperar uma sandbox padrão. Erro da API de gerenciamento de sandbox: Algo deu errado. Tente novamente. |
| `2108-500` | Obter erro de sandboxes ativas | Ocorreu um problema ao recuperar uma sandbox ativa. Erro da API de gerenciamento de sandbox: Algo deu errado. Tente novamente. |
...
| `2112-400` | Cabeçalhos com valores diferentes não são permitidos | Os cabeçalhos x-sandbox-name e x-sandbox-id devem ter o valor * para consulta entre sandboxes. Atualize os cabeçalhos e tente novamente. |
...
| `2116-500` | Erro interno | Ocorreu um erro interno durante a decodificação offline do token de portador para resolução de escopo. |
| `2117-500` | Erro interno | Ocorreu um erro interno ao verificar as permissões para o usuário. Tente novamente. Se o problema persistir, entre em contato com o suporte ao cliente. |
...
| `2120-403` | Proibido | Você não tem permissões suficientes para executar a operação. Entre em contato com o administrador para obter as permissões e tente novamente. |
...
| `2200-500` | Erro de solicitação de serviço externo | Houve um problema ao chamar o serviço de Catálogo para validar o esquema. |
| `2250-500` | Erro de solicitação de serviço externo | Ocorreu um problema ao chamar o serviço de Preparo de dados para validar o mapeamento. |
