...
# [!DNL Graph Simulation] Guia da interface do usuário
...
Leia este documento para saber como usar [!DNL Graph Simulation] para entender melhor o comportamento do gráfico de identidade e como o algoritmo de gráfico funciona.

## Conheça o [!DNL Graph Simulation] interface {#interface}

Você pode acessar [!DNL Graph Simulation] na interface do Adobe Experience Platform. Selecionar **[!UICONTROL Identidades]** na navegação à esquerda e selecione **[!UICONTROL Simulação de gráfico]** no cabeçalho superior.
...
A variável [!DNL Graph Simulation] A interface do pode ser dividida em três seções:
...
![O botão Add events foi selecionado.](../images/graph-simulation/add-events.png)
...
![A janela Event #1 com uma interface vazia.](../images/graph-simulation/event-one.png)

>[!TIP]
>
>O valor de identidade que você insere durante [!DNL Graph Simulation] Os exercícios do não precisam ser valores de identidade reais e podem ser espaços reservados simples.
...
Selecionar **[!UICONTROL Carregar exemplo]** para configurar um gráfico de exemplo com um algoritmo predefinido e uma configuração de evento.

![A opção Load example foi selecionada.](../images/graph-simulation/load-example.png)
...
![Os eventos e o algoritmo configurados para telefone inválido.](../images/graph-simulation/example-loaded.png)

Quando terminar, selecione **[!UICONTROL Simular]**.

![Um gráfico de exemplo simulado para telefone inválido.](../images/graph-simulation/example-simulated.png)
...
![Painel Eventos usando a versão do modo de texto.](../images/graph-simulation/text-version.png)
...
![O ícone do evento de edição selecionado.](../images/graph-simulation/edit.png)
...
![O ícone Excluir evento selecionado.](../images/graph-simulation/delete.png)

## Configurar algoritmo {#configure-algorithm}

>[!IMPORTANT]
>
>O algoritmo que você configura determina como o Serviço de identidade trata os namespaces inseridos em seus eventos. Qualquer configuração que você definir na variável [!DNL Graph Simulation UI] não são salvas nas configurações de identidade.
...
Uma linha de configuração vazia é exibida. Primeiro, insira o mesmo namespace que você usou para os eventos. Nesse caso, comece inserindo o Email. Depois de inserir o namespace, as colunas para [!UICONTROL Símbolo de identidade] e [!UICONTROL Tipo de identidade] é preenchido automaticamente.
...
* **Namespace exclusivo**: se um namespace for designado como exclusivo, o Serviço de identidade gerará gráficos com a ressalva de que apenas uma identidade com determinado namespace exclusivo pode existir. Por exemplo, se o namespace de email for designado como um namespace exclusivo, um gráfico só poderá ter uma identidade com Email. Se houver mais de uma identidade com o namespace de email, o link mais antigo será removido.
...
No gráfico de exemplo abaixo, existe uma linha pontilhada entre `{Email: tom@acme.com}` e `{ECID: 111}` pelos seguintes motivos:
...
Ao ler este documento, agora você sabe como usar o [!DNL Graph Simulation] para entender melhor como seus dados de identidade são tratados com base em um conjunto específico de regras e configurações. Para obter mais informações, leia os seguintes documentos:
