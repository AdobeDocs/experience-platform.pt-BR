...
| `status` | O status da participação do segmento como parte da solicitação atual. Deve ser igual a um dos seguintes valores conhecidos: <ul><li>`realized`: Entidade se qualifica para o segmento.</li><li>`exited`: A entidade está saindo do segmento.</li></ul> |
