...
O schema que você está fazendo neste tutorial usa a classe Perfil individual XDM. O Perfil XDM Individual é uma classe padrão fornecida pela Adobe para definir o comportamento do registro. Mais informações sobre o comportamento podem ser encontradas nos [fundamentos da composição](../schema/composition.md)do schema.
...
Uma solicitação bem-sucedida retorna o Status de Resposta HTTP 201 (Criado) com um corpo de resposta que contém os detalhes do schema recém-criado, incluindo `$id`, `meta:altIt`e `version`. Esses valores são somente leitura e são atribuídos pelo Registro do Schema.
...
Para visualização de seu schema recém-criado, execute uma solicitação de pesquisa (GET) usando o `meta:altId` URL codificado `$id` ou o URL do schema.
...
Esta solicitação atualiza (PATCH) o schema Membros de Fidelidade para incluir os campos na combinação &quot;perfil-pessoa-detalhes&quot;.

Ao adicionar a combinação &quot;perfil-pessoa-detalhes&quot;, o schema Membros da Fidelidade agora captura informações sobre membros do programa de fidelidade, como seu nome, sobrenome e aniversário.
...
Agora é possível adicionar outra mixagem padrão repetindo as etapas usando outra mixagem.

>[!TIP] Vale a pena rever todas as combinações disponíveis para se familiarizar com os campos incluídos em cada uma. Você pode lista (GET) todas as combinações disponíveis para uso com uma classe específica executando uma solicitação em relação a cada um dos container &quot;global&quot; e &quot;locatário&quot;, retornando somente as combinações nas quais o campo &quot;meta:pretendidoToExtend&quot; corresponde à classe que você está usando. Nesse caso, é a classe de Perfil individual XDM, de modo que o Perfil individual XDM `$id` é usado:
...
Esta solicitação atualiza (PATCH) o schema Membros da fidelidade para incluir os campos na combinação &quot;Detalhes pessoais do perfil&quot;, adicionando os campos &quot;endereço residencial&quot;, &quot;endereço de email&quot; e &quot;telefone residencial&quot; ao schema.
...
O Registro do Schema é responsável por isso, permitindo que você defina suas próprias combinações dentro do container do locatário. Essas combinações são exclusivas da sua organização e não são visíveis nem editáveis por ninguém fora da sua Organização IMS.

Para criar (POST) uma nova combinação, sua solicitação deve incluir um `meta:intendedToExtend` campo contendo o campo `$id` para as classes base com as quais a mistura é compatível, juntamente com as propriedades que a mistura incluirá.
...
Essa solicitação cria uma nova combinação que tem um objeto de &quot;fidelidade&quot; que contém quatro campos específicos do programa de fidelidade: &quot;loyaltyId&quot;, &quot;loyaltyLevel&quot;, &quot;loyaltyPoints&quot; e &quot;MemberSince&quot;.
...
Esta solicitação atualiza (PATCH) o schema Membros de Fidelidade para incluir os campos na nova combinação &quot;Detalhes do Membro de Fidelidade&quot;.
...
### Visualização do schema atual

Agora é possível executar uma solicitação GET para visualização do schema atual e ver como as combinações adicionadas contribuíram para a estrutura geral do schema.
...
A combinação de Fidelidade criada contém propriedades de fidelidade específicas que podem ser úteis em outros schemas. Por exemplo, os dados podem ser assimilados como parte de um evento de experiência ou usados por um schema que implementa uma classe diferente. Nesse caso, faz sentido salvar a hierarquia de objetos como um tipo de dados para facilitar a reutilização da definição em outro lugar.

Os tipos de dados permitem que você defina uma hierarquia de objetos uma vez e faça referência a ela em um campo da mesma forma que faria para qualquer outro tipo escalar.

Em outras palavras, os tipos de dados permitem o uso consistente de estruturas de vários campos, com mais flexibilidade do que combinações, pois podem ser incluídos em qualquer lugar de um schema adicionando-os como o &quot;tipo&quot; de um campo.
...
Uma solicitação bem-sucedida retorna o Status de Resposta HTTP 201 (Criado) com um corpo de resposta que contém os detalhes do tipo de dados recém-criado, incluindo o `$id`, `meta:altIt`e `version`. Esses valores são somente leitura e são atribuídos pelo Registro do Schema.
...
Agora que o tipo de dados Detalhes da Fidelidade foi criado, você pode atualizar (PATCH) o campo &quot;fidelidade&quot; no mixin criado para fazer referência ao tipo de dados no lugar dos campos que estavam lá anteriormente.
...
Executar uma solicitação GET para pesquisar o schema agora mostra a referência ao tipo de dados em &quot;properties/_{TENANT_ID}&quot;, como visto aqui:
...
A solicitação a seguir define um descritor de identidade no campo &quot;loyaltyId&quot;. Isso instrui a Experience Platform a usar o identificador exclusivo do membro do programa de fidelidade (neste caso, o endereço de email do membro) para ajudar a unir informações sobre o indivíduo.
...
>[!NOTE] Você pode lista valores &quot;xdm:namespace&quot; disponíveis ou criar novos valores usando a API [do Serviço de](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/id-service-api.yaml)identidade. O valor para &quot;xdm:property&quot; pode ser &quot;xdm:code&quot; ou &quot;xdm:id&quot;, dependendo da &quot;xdm:namespace&quot; usada.

**Resposta**

Uma resposta bem-sucedida retorna o Status HTTP 201 (Criado) com um corpo de resposta contendo os detalhes do descritor recém-criado, incluindo seu `@id`. O campo `@id` é somente leitura atribuído pelo Registro do Schema e é usado para fazer referência ao descritor na API.
...
Ao adicionar a tag &quot;união&quot; ao `meta:immutableTags` atributo, você pode ativar o schema Membros da fidelidade para uso pelo Perfil do cliente em tempo real.

Para obter mais informações sobre como trabalhar com visualizações de união, consulte a seção sobre [união](../api/unions.md) no guia do desenvolvedor do Registro de Schemas.
...
A resposta mostra que a operação foi executada com êxito, e o schema agora contém um atributo de nível superior, `meta:immutableTags`, que é uma matriz que contém o valor, &quot;união&quot;.
...
Agora você adicionou com êxito seu schema à união do Perfil individual XDM. Para ver uma lista de todos os schemas que fazem parte da mesma união, é possível executar uma solicitação GET usando parâmetros de query para filtrar a resposta.

Usando o parâmetro `property` query, você pode especificar que apenas schemas que contenham um `meta:immutableTags` campo que tenha um valor `meta:class` `$id` igual ao da classe de Perfil Individual XDM sejam retornados.
...
A solicitação de exemplo abaixo retorna todos os schemas que fazem parte da união do Perfil individual XDM.
...
A resposta é uma lista filtrada de schemas, contendo apenas aqueles que atendem a ambos os requisitos. Lembre-se de que ao usar vários parâmetros de query, uma relação E é assumida. O formato da resposta da lista depende do cabeçalho Aceitar enviado na solicitação.
...
O schema implementa a classe de Perfil individual XDM e combina várias mixagens; trazendo informações sobre os membros da fidelidade usando as combinações padrão &quot;Detalhes da pessoa&quot; e &quot;Detalhes pessoais&quot;, bem como através de uma combinação &quot;Detalhes da fidelidade&quot; definida durante o tutorial.
