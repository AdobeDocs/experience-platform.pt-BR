...
>[!NOTE]
>
>Para adicionar `lastQualificationTime` às suas exportações de arquivo, você precisa inserir manualmente o valor `xdm: segmentMembership.ups.seg_id.lastQualificationTime` no campo de origem, conforme mostrado abaixo. Você também pode editar o campo de destino como `lastQualificationTime` ou qualquer outro valor que queira nomear para essa coluna. Como esta é uma funcionalidade beta, a sintaxe do valor `xdm: segmentMembership.ups.seg_id.lastQualificationTime` pode mudar no futuro.
