---
description: Saiba como usar o Destination SDK para configurar um destino de Armazenamento Azure Blob com opções personalizadas de formatação de arquivo e configuração personalizada de nome de arquivo.
title: (Beta) Configure um destino do Armazenamento Azure Blob com opções personalizadas de formatação de arquivos e configuração personalizada de nome de arquivo.
...
# (Beta) Configure um [!DNL Azure Blob Storage] destino com opções de formatação de arquivo personalizadas e configuração de nome de arquivo personalizado
...
Esta página descreve como usar o Destination SDK para configurar um [!DNL Azure Blob Storage] destino com personalizado [opções de formatação de arquivo](../../server-and-file-configuration.md#file-configuration) e um [configuração do nome do arquivo](../../file-based-destination-configuration.md#file-name-configuration).

Esta página mostra todas as opções de configuração disponíveis para os destinos de Armazenamento do Azure Blob. Você pode editar as configurações mostradas nas etapas abaixo ou excluir determinadas partes das configurações, conforme necessário.
...
Comece usando o `/destination-server` endpoint para criar uma configuração de servidor e arquivo. Para obter descrições detalhadas dos parâmetros na solicitação HTTP, leia o [especificações de configuração de servidor e arquivo para destinos com base em arquivo](../../server-and-file-configuration.md#blob-example) e o [configurações de formatação de arquivo](../../server-and-file-configuration.md#file-configuration).
...
![Gravação de tela mostrando a página de catálogo de destinos com um cartão de destino selecionado.](../../assets/blob-destination-card.gif)
...
Ao ler este artigo, agora você sabe criar um [!DNL Azure Blob Storage] ao usar Destination SDK. Em seguida, sua equipe pode usar o [fluxo de trabalho de ativação para destinos com base em arquivo](../../../ui/activate-batch-profile-destinations.md) para exportar dados para o destino.