...
title: (Beta) Criar um fluxo de dados para uma origem da Integração do OneTrust usando a API do Serviço de Fluxo
...
# (Beta) Criar um fluxo de dados para um [!DNL OneTrust Integration] fonte usando o [!DNL Flow Service] API
...
## Connect [!DNL OneTrust Integration] para a plataforma usando a [!DNL Flow Service] API

>[!NOTE]
>
>O [!DNL OneTrust Integration] As especificações da API estão sendo compartilhadas com o Adobe para assimilação de dados.

O tutorial a seguir o orienta pelas etapas para criar um [!DNL OneTrust Integration] conexão de origem e criar um fluxo de dados para trazer [!DNL OneTrust Integration] dados para a plataforma usando a [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

### Criar uma conexão base {#base-connection}

Uma conexão base retém informações entre a fonte e a Plataforma, incluindo as credenciais de autenticação da fonte, o estado atual da conexão e a ID de conexão base exclusiva. A ID de conexão básica permite explorar e navegar pelos arquivos da fonte e identificar os itens específicos que deseja assimilar, incluindo informações sobre os tipos e formatos de dados.

Para criar uma ID de conexão base, faça uma solicitação de POST para a variável `/connections` endpoint enquanto fornece seu [!DNL OneTrust Integration] credenciais de autenticação como parte do corpo da solicitação.
...
| `auth.params.` | Contém as credenciais necessárias para autenticar sua fonte. O host raiz e o token de acesso para se conectar à API. |
| `auth.params.host` | O ambiente a partir do qual o [!DNL OneTrust Integration] Os dados do precisam ser obtidos do. |
| `auth.params.accessToken` | O token de acesso que corresponde ao seu [!DNL OneTrust Integration] conta. |
...
>[!NOTE]
>
>A carga de resposta JSON abaixo está oculta para brevidade. Selecione Clique em me para ver a carga da resposta.
...
| `baseConnectionId` | A ID de conexão básica de [!DNL OneTrust Integration]. Essa ID foi gerada em uma etapa anterior. |
| `connectionSpec.id` | A ID da especificação de conexão que corresponde à sua origem. |
| `data.format` | O formato do [!DNL OneTrust Integration] dados que você deseja assimilar. Atualmente, o único formato de dados compatível é `json`. |
...
Um esquema XDM de destino pode ser criado executando-se uma solicitação de POST para a [API do Registro de Schema](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/schema-registry.yaml).
...
| `data.format` | O formato do [!DNL OneTrust Integration] dados que você deseja trazer para a plataforma. |
...
O último passo para trazer dados do [!DNL OneTrust Integration] para Platform é criar um fluxo de dados. Por enquanto, você terá os seguintes valores obrigatórios preparados:
