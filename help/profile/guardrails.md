...
Ao definir seu modelo de dados, é recomendável permanecer dentro das grades de proteção fornecidas para garantir o desempenho adequado e evitar erros do sistema. As medidas de proteção fornecidas neste documento incluem dois tipos de limite:
...
As seguintes medidas de proteção referem-se ao tamanho dos dados e são recomendadas para garantir que os dados possam ser assimilados, armazenados e consultados conforme planejado.

>[!NOTE]
>
>O tamanho dos dados será medido como dados descompactados no JSON no momento da ingestão.

### Medidas de proteção de entidade primária

| Grade de Proteção | Limite | Tipo de limite | Descrição |
| --- | --- | --- | --- |
| Tamanho máximo por fragmento de perfil | 10 KB | Suave | **O tamanho máximo recomendado de um fragmento de perfil é de 10 KB.** Inserir fragmentos de perfil maiores afetará o desempenho do sistema. Por exemplo, carregar um conjunto de dados CRM pesado, onde alguns fragmentos de perfil têm 50kB, resultará em desempenho degradado do sistema. |
| Tamanho máximo absoluto por fragmento de perfil | 1 MB | Grave | **O tamanho máximo absoluto de um fragmento de perfil é de 1MB.** A assimilação falhará ao tentar carregar um fragmento de perfil maior que 1MB. |
