---
keywords: Experience Platform;home;popular topics;Serviço de segmentação;segmentação;serviço de segmentação;guia do usuário;guia ui;guia de segmentação de ID;construtor de segmentos;percebido;existente;sair;
...
Na [[!DNL Experience Platform] IU](http://platform.adobe.com/), selecione **[!UICONTROL Segmentos]** no painel de navegação esquerdo para abrir a guia **[!UICONTROL Visão geral]**. Esta guia fornece links para a documentação e vídeos para ajudá-lo a entender e começar a trabalhar com segmentos.

![](../images/ui/overview/segment-overview.png)

## Procurar

Selecione a guia **[!UICONTROL Procurar]** para ver uma lista de todas as definições de segmento para a sua Organização IMS.
...
O detalhamento mostra um gráfico de barras que descreve a porcentagem de perfis que pertencem a cada um dos seguintes status: [!UICONTROL Digitado], [!UICONTROL Realizado] e [!UICONTROL Saindo].
...
Alternar **[!UICONTROL Adicionar todos os segmentos à programação]** permitirá a segmentação programada. Mais informações sobre a segmentação programada podem ser encontradas na seção [segmentação programada deste guia do usuário](#scheduled-segmentation).

Selecionar **[!UICONTROL Criar segmento]** levará você ao Construtor de segmentos. Para saber mais sobre como criar segmentos, leia a seção sobre [como criar um segmento no guia do usuário](#create-segment).
...
Além disso, você tem a opção de editar o segmento. Selecionar **[!UICONTROL Editar segmento]** o levará até [!DNL Segment Builder]. Para obter informações mais detalhadas sobre como usar a área de trabalho [!DNL Segment Builder], leia o [[!DNL Segment Builder] guia do usuário](./segment-builder.md).

### Audiência total no segmento

A seção **[!UICONTROL audiência total no segmento]** mostra o número total de perfis que se qualificam para o segmento.
...
>[!NOTE]
>
> Os destinos são um recurso disponível com [!DNL Real-time Customer Data Platform] e permitem exportar dados para plataformas externas. Para obter mais informações sobre destinos, leia a [visão geral de destinos](../../destinations/home.md). Para saber como ativar um segmento em um destino, leia o guia [sobre como ativar segmentos em um destino](../../destinations/ui/activate-destinations.md).

### Amostras de perfil

Abaixo está uma amostra de perfis que se qualificam para o segmento, detalhando informações, incluindo a ID [!DNL Profile], o nome, o sobrenome e o email pessoal.
...
Selecionar **[!UICONTROL Criar segmento]** no canto superior direito abre a área de trabalho [!DNL Segment Builder], onde você pode começar a criar uma definição de segmento.
...
### Habilitar segmentação agendada {#enable-scheduled-segmentation}
...
A segmentação contínua é a capacidade de fazer segmentação em [!DNL Platform] em tempo quase real, enquanto se concentra na riqueza de dados. Com a segmentação de fluxo contínuo, a qualificação de segmentos agora acontece à medida que os dados chegam [!DNL Platform], diminuindo a necessidade de programar e executar tarefas de segmentação.

Para obter mais informações sobre a segmentação de streaming, consulte [guia do usuário de segmentação de streaming](./streaming-segmentation.md).

>[!NOTE]
>
>Para que a segmentação de fluxo funcione, será necessário ativar a segmentação programada para a organização. Para obter detalhes sobre como ativar a segmentação programada, consulte [a seção de segmentação de streaming neste guia do usuário](#scheduled-segmentation).
...
A interface do usuário [!DNL Segmentation Service] fornece um fluxo de trabalho avançado que permite isolar audiências comercializáveis dos dados [!DNL Real-time Customer Profile].
