...
Ao transmitir dados para a Adobe Experience Platform, fazer várias chamadas HTTP pode ser caro. Por exemplo, em vez de criar 200 solicitações HTTP com cargas de 1 KB, é muito mais eficiente criar 1 solicitação HTTP com 200 mensagens de 1 KB cada, com uma única carga de 200 KB. Quando usado corretamente, agrupar várias mensagens em uma única solicitação é uma excelente maneira de otimizar os dados que estão sendo enviados para a plataforma Experience.

Este documento fornece um tutorial para enviar várias mensagens para a plataforma Experience em uma única solicitação HTTP usando a assimilação de streaming.

## Introdução

Este tutorial requer um entendimento prático da assimilação de dados da plataforma Adobe Experience. Antes de iniciar este tutorial, reveja a seguinte documentação:

- [Visão geral](../home.md)da ingestão de dados: Abrange os conceitos principais de assimilação de dados da plataforma de experiência, incluindo métodos de ingestão e conectores de dados.
- [Visão geral](../streaming-ingestion/overview.md)da assimilação de transmissão: O fluxo de trabalho e os blocos componentes da assimilação de streaming, como conexões de streaming, conjuntos de dados, Perfil individual XDM e ExperienceEvent XDM.

Este tutorial também exige que você tenha concluído o tutorial [Autenticação para a plataforma](../../tutorials/authentication.md) Adobe Experience para realizar com êxito chamadas para APIs de plataforma. A conclusão do tutorial de autenticação fornece o valor para o cabeçalho de Autorização necessário para todas as chamadas de API neste tutorial. O cabeçalho é mostrado nas chamadas de amostra da seguinte maneira:
...
Primeiro, você deve criar uma conexão de streaming antes de poder start os dados de streaming para a plataforma Experience. Leia o guia [Criar uma conexão](./create-streaming-connection.md) de streaming para saber como criar uma conexão de streaming.
...
Você pode obter a ID de um conjunto de dados existente usando a interface do usuário da plataforma ou usando uma operação de listagem na API. A ID do conjunto de dados pode ser encontrada na Plataforma [de](https://platform.adobe.com) experiência indo até a guia **Conjuntos** de dados, clicando no conjunto de dados para o qual deseja obter a ID e copiando a string do campo ID **do** Conjunto de dados na guia **Informações** . Consulte a visão geral [do serviço de](../../catalog/home.md) catálogo para obter informações sobre como recuperar conjuntos de dados usando a API.
...
- O `imsOrgId` campo no cabeçalho da mensagem deve corresponder à definição de entrada. Se a carga da solicitação não incluir um `imsOrgId` campo, o DCCS (Data Collection Core Service) adicionará o campo automaticamente.
- O cabeçalho da mensagem deve fazer referência a um schema XDM existente criado na interface do usuário da plataforma.
- O `datasetId` campo precisa fazer referência a um conjunto de dados existente na Plataforma e seu schema precisa corresponder ao schema fornecido no `header` objeto dentro de cada mensagem incluída no corpo da solicitação.
...
A primeira mensagem foi enviada com êxito para a Plataforma e não é afetada pelos resultados das outras mensagens. Como resultado, ao tentar reenviar as mensagens com falha, não é necessário reincluir essa mensagem.
...
As mensagens que passam na validação do DCCS são transmitidas para a Plataforma. Na Plataforma, as mensagens em lote são testadas pela validação de fluxo contínuo antes de serem ingeridas no lago de dados. O status dos lotes, com ou sem êxito, é exibido no conjunto de dados especificado por `{DATASET_ID}`.

Você pode visualização o status das mensagens em lote que são transmitidas com êxito para a Plataforma usando a interface do usuário [da plataforma de](https://platform.adobe.com) experiência indo até a guia **Conjuntos** de dados, clicando no conjunto de dados para o qual você está fazendo streaming e verificando a guia Atividade **do** Conjunto de dados.

As mensagens em lote que passam pela validação de streaming na Plataforma são ingeridas no lago de dados. As mensagens estão disponíveis para análise ou exportação.

## Próximas etapas

Agora que você sabe como enviar várias mensagens em uma única solicitação e verificar quando as mensagens são ingeridas com êxito no conjunto de dados do público alvo, é possível fazer o start do streaming dos seus próprios dados para a Plataforma. Para obter uma visão geral de como query e recuperação de dados ingeridos da Plataforma, consulte o guia Acesso [a](../../data-access/tutorials/dataset-data.md) dados.
...
| 500 | Erro ao processar a carga. Consulte o corpo da resposta para obter uma mensagem de erro mais específica (por exemplo, schema de carga de mensagem não especificado ou não corresponde à definição XDM na Plataforma). |
