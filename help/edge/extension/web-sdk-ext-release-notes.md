...
* Contém a versão 2.16.0 do SDK da Web da Adobe Experience Platform.
...
* Adição da configuração para [`onBeforeLinkClickSend`](../fundamentals/configuring-the-sdk.md#onBeforeLinkClickSend) função de retorno de chamada.

**Correções e melhorias**

* Correção de um problema que fazia com que o clique em elementos dentro de uma tag de âncora não funcionasse quando a função **[!UICONTROL Redirecionar com identidade]** foi usada.
...
* Correção de um problema em que os usuários sem acesso a conjuntos de dados não podiam editar a configuração da extensão.
* Adição de suporte para superfícies no `sendEvent` ação.

Contém a versão 2.14.0 do SDK da Web da Adobe Experience Platform.


## Versão 2.14.1 - 13 de outubro de 2022

* Correção de um problema em que o SDK da Web não honrava a ID do serviço da Experience Cloud ID.

Contém a versão 2.13.1 da Biblioteca do SDK da Web da Adobe Experience Platform.

## Versão 2.14.0 - 28 de setembro de 2022

* Adicionado um novo `targetMigrationEnabled` configuração que permite a migração de página por página inteira.
* Adição de uma ação de resposta de aplicação para ativar implementações híbridas servidor-cliente.
* Adicionada a opção de contexto de dicas de cliente de agente-usuário de alta entropia.
...
* Adicionado **[!UICONTROL Redirecionar com identidade]** para permitir o compartilhamento de identidade entre domínios.
* Adição de links de documentação ao `sendEvent` ação.
* Atualização da biblioteca da interface do usuário do React Spectrum.
...
* Correção do erro de comando de configuração da versão 2.11.0.

Contém a versão 2.10.0 da Biblioteca do SDK da Web da Adobe Experience Platform.

## Versão 2.11.0 - 22 de abril de 2022

* Desempenho aprimorado da interface do usuário de tags.
* Adicione seletores de sandbox à configuração de extensão de datastreams.
