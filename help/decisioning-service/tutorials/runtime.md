...
Este documento fornece um tutorial para trabalhar com os serviços de tempo de execução do serviço de decisão usando as APIs da plataforma Adobe Experience.

## Introdução

Este tutorial requer um entendimento prático dos serviços da plataforma de experiência envolvidos na tomada de decisões e na determinação da próxima melhor oferta a ser apresentada durante as experiências do cliente. Antes de iniciar este tutorial, reveja a documentação do seguinte:

- [Serviço](./../home.md)de decisão: Fornece a estrutura para adicionar e remover ofertas e criar algoritmos para escolher o melhor para apresentar durante a experiência do cliente.
...
- [Linguagem do Query do Perfil (PQL)](../../segmentation/pql/overview.md): O PQL é usado para definir regras e filtros.
...
>[!NOTE] Para obter mais informações sobre caixas de proteção na Plataforma, consulte a documentação [de visão geral da](../../tutorials/authentication.md)caixa de proteção.
...
A ativação das entidades de lógica comercial acontece automática e continuamente. Assim que uma nova opção for salva no repositório e for marcada como &quot;aprovada&quot;, ela será candidata à inclusão do conjunto de opções disponíveis. Assim que uma regra de decisão for atualizada, o conjunto de regras será remontado e preparado para execução em tempo de execução. Nesta etapa de ativação automática, todas as restrições definidas pela lógica de negócios que não dependem do contexto de tempo de execução serão avaliadas. Os resultados desta etapa de ativação são enviados para um cache onde estão disponíveis para o tempo de execução do Serviço de Decisão.
...
A REST API é uma das rotas para aplicativos em execução na plataforma, a fim de obter a melhor experiência a seguir com base nas regras, modelos e restrições que a organização definiu para seus usuários. As aplicações enviam uma das identidades do perfil (ID do perfil e namespace de identidade) para as quais o serviço de decisão irá procurar o perfil e as informações serão usadas para aplicar a lógica comercial. Dados de contexto adicionais podem ser passados para a solicitação e, se especificados nas regras de negócios, serão incluídos nos dados para tomar a decisão.
...
- **`xdm:dryRun`** - Quando o valor desta propriedade opcional for definido como true, o pedido de decisão obedecerá a restrições de limitação, mas não determinará efetivamente esses contadores, é de esperar que o chamador nunca tencione apresentar a proposta ao perfil. O Serviço de decisão não registrará a proposta como um evento de decisão XDM oficial e ela não aparecerá nos conjuntos de dados do relatórios. O valor padrão desta propriedade é falso e quando a propriedade é omitida a decisão não é considerada uma execução de teste e, portanto, deve ser apresentada ao usuário final.
