...
title: Criar um fluxo de dados para fontes CRM usando a API do serviço de fluxo
...
# Crie um fluxo de dados para fontes CRM usando o [!DNL Flow Service] API

Este tutorial aborda as etapas para recuperar dados de uma fonte de CRM e trazê-los para a plataforma usando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>Para criar um fluxo de dados, você já deve ter uma ID de conexão base válida com uma fonte CRM. Se você não tiver essa ID, consulte a [visão geral das fontes](../../../home.md#customer-relationship-management) para obter uma lista de fontes CRM com as quais você pode criar uma conexão básica.
...
| `connectionSpec.id` | A ID de especificação de conexão associada ao seu sistema de CRM específico de terceiros. Consulte a [apêndice](#appendix) para obter uma lista de IDs de especificação de conexão. |
...
| `transformations.params.deltaColum` | A coluna designada usada para diferenciar dados novos e existentes. Os dados incrementais serão assimilados com base no carimbo de data e hora da coluna selecionada. O formato compatível para `deltaColumn` é `yyyy-MM-dd HH:mm:ss`. Se você estiver usando o Microsoft Dynamics, o formato compatível para `deltaColumn` é `yyyy-MM-ddTHH:mm:ssZ`. |
