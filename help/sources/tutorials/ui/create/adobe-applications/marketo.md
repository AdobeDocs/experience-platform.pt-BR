...
As seções a seguir fornecem diretrizes adicionais que podem ser seguidas ao usar o [!DNL Marketo] fonte.

### Mensagens de erro na interface do usuário {#error-messages}
...
A autenticação será negada se a sua [!DNL Munchkin ID] não está mapeado para a organização da plataforma que você está usando. Configure o mapeamento entre [!DNL Munchkin ID] e sua organização usando o [[!DNL Marketo] interface](https://app-sjint.marketo.com/#MM0A1).

![Uma mensagem de erro que exibe que a instância do Marketo não está mapeada corretamente para a organização do Adobe.](../../../../images/tutorials/create/marketo/munchkin-not-mapped.png)

#### Identidade primária ausente

Um fluxo de dados não será salvo e assimilado se uma identidade primária estiver ausente. Certifique-se de que [existe uma identidade primária dentro do esquema XDM](../../../../../xdm/tutorials/create-schema-ui.md), antes de tentar configurar um fluxo de dados.

![Uma mensagem de erro que exibe que a identidade primária está ausente do esquema XDM.](../../../../images/tutorials/create/marketo/no-primary-identity.png)
