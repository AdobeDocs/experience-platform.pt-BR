...
| Incluir arquivo de manifesto em exportações para destinos beta de armazenamento na nuvem | Agora é possível incluir um arquivo JSON de manifesto que contenha informações sobre o local de exportação, o tamanho da exportação e muito mais ao exportar dados para os seis destinos beta de armazenamento na nuvem: [(Beta) Amazon S3](/help/destinations/catalog/cloud-storage/amazon-s3.md), [(Beta) Azure Blob](/help/destinations/catalog/cloud-storage/azure-blob.md), [(Beta) Azure Data Lake Storage Gen2](/help/destinations/catalog/cloud-storage/adls-gen2.md), [(Beta) Data Landing Zone](/help/destinations/catalog/cloud-storage/data-landing-zone.md), [(Beta) Google Cloud Storage](/help/destinations/catalog/cloud-storage/google-cloud-storage.md) e [(Beta) SFTP](/help/destinations/catalog/cloud-storage/sftp.md). <br><br> Obtenha mais informações na seção **[!UICONTROL Detalhes do destino]** das páginas vinculadas acima. |
...
Para obter mais informações sobre XDM na Platform, leia a [Visão geral do sistema de XDM](../../xdm/home.md).
...
| Recurso | Descrição |
| --- | --- |
| Suporte para IDs de parceiro nos aplicativos da Adobe Experience Cloud [!BADGE Beta]{type=Informative} | As IDs de parceiro agora estão disponíveis no Identity Service. IDs de parceiro são identificadores usados por parceiros de dados para representar pessoas. Na Real-time Customer Data Platform, as IDs de parceiro são usadas principalmente para uma ativação de público-alvo expandida e o enriquecimento de dados. As IDs de parceiro não são armazenadas no gráfico de identidade. Para obter mais informações, leia a documentação sobre [tipos de identidade](../../identity-service/namespaces.md#identity-types). |
...
| Recurso | Descrição |
| --- | --- |
| Calcular estatísticas em nível de coluna nos conjuntos de dados ADLS | O comando `ANALYZE TABLE` recebeu dois comandos de SQL adicionais: `COMPUTE STATISTICS` e `SHOW STATISTICS`. Agora é possível calcular estatísticas para um subconjunto de um conjunto de dados ADLS ou para determinadas colunas nesse conjunto de dados. Para obter mais informações, leia o [guia de computação de estatísticas do conjunto de dados](../../query-service/essential-concepts/dataset-statistics.md). |
