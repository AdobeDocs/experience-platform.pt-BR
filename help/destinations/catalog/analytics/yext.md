...
description: A extensão Rastreamento de conversão do Yext é um destino de análise no Adobe Experience Platform. Para obter mais informações sobre a funcionalidade de extensão, consulte a página de extensão no Adobe Exchange.
...
A extensão [!DNL Yext Conversion Tracking] permite medir conversões que podem ser atribuídas ao uso de produtos do Yext.
...
Para obter informações sobre as opções de configuração de extensão e o suporte de instalação, consulte a página [Rastreamento de conversão do Yext no Adobe Exchange](https://exchange.adobe.com/experiencecloud.details.103174.yext-conversion-tracking.html).
