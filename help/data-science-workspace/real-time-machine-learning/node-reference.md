...
# Guia de referência dos nós de aprendizado de máquina em tempo real
...
O nó abaixo usa `"import": "map"` para importar o nome do método como uma string nos parâmetros, seguido por, inserindo os parâmetros como uma função de mapa. O exemplo abaixo faz isso usando `{"arg": {"Desktop": 1, "Mobile": 0}, "na_action": 0}`. Depois de colocar o mapa no lugar, você tem a opção de definir `inplace` como verdadeiro ou falso. Defina `inplace` como `True` ou `False` com base em se deseja aplicar a transformação no local ou não. Por padrão, `"inplace": False` cria uma nova coluna. O suporte para fornecer um novo nome de coluna está definido para ser adicionado em uma versão subsequente. A última linha `cols` pode ser um nome de coluna único ou uma lista de colunas. Especifique as colunas nas quais deseja aplicar a transformação. Neste exemplo, `device` é especificado.
...
| params.model | Caminho de importação absoluto para o modelo (string), por exemplo: &quot;sklearn.linear_model.LogismRegression&quot;. |
...
A próxima etapa é criar um nó para usar na pontuação de um modelo de aprendizado de máquina em tempo real. Visite o tutorial sobre como [marcar um modelo](./node-reference.md) de aprendizado de máquina em tempo real para obter mais informações.
