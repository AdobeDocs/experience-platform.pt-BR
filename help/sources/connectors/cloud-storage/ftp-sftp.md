---
keywords: Experience Platform;home;popular topics
solution: Experience Platform
title: Conector FTP e SFTP
topic: overview
translation-type: tm+mt
source-git-commit: 0ed2ed3b08f262100746f255a78c248a1748eb5e
workflow-type: tm+mt
source-wordcount: '0'
ht-degree: 0%

---


# (Beta) Conector FTP e SFTP

A plataforma Adobe Experience fornece conectividade nativa para provedores de nuvem como AWS, plataforma Google Cloud e Azure, permitindo que você traga seus dados desses sistemas.

As fontes de armazenamentos na nuvem podem trazer seus próprios dados para a Plataforma sem a necessidade de baixar, formatar ou fazer upload. Os dados ingeridos podem ser formatados como XDM JSON, XDM parquet ou delimitados. Cada etapa do processo é integrada ao fluxo de trabalho de Fontes. A plataforma permite trazer dados de um servidor FTP ou SFTP por lotes.

A documentação abaixo fornece informações sobre como conectar um FTP ou um servidor STFP à plataforma usando APIs ou a interface do usuário:

## Conectar FTP e SFTP à plataforma usando APIs

- [Criar um conector FTP ou SFTP usando a API de serviço de fluxo](../../tutorials/api/create/cloud-storage/sftp.md)
- [Explore um sistema de armazenamento em nuvem usando a API de Serviço de Fluxo](../../tutorials/api/explore/cloud-storage.md)
- [Coletar dados de armazenamento na nuvem usando a API de Serviço de Fluxo](../../tutorials/api/collect/cloud-storage.md)

## Conectar FTP ou SFTP à plataforma usando a interface do usuário

- [Criar um conector de origem FTP ou SFTP na interface do usuário](../../tutorials/ui/create/cloud-storage/ftp-sftp.md)
- [Configurar um fluxo de dados para um conector de armazenamento em nuvem na interface do usuário](../../tutorials/ui/dataflow/batch/cloud-storage.md)