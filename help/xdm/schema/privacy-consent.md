...
# [!DNL Consents & Preferences] visão geral do tipo de dados

O tipo de [!DNL Privacy/Marketing Preferences (Consent)] dados (a seguir denominado &quot;tipo de[!DNL Consents & Preferences] dados&quot;) é um tipo de dados [!DNL Experience Data Model] (XDM) destinado a suportar a coleta de permissões e preferências do cliente geradas pelas Plataformas de gerenciamento de consentimento (CMPs) e outras fontes de suas operações de dados.
...
O JSON a seguir mostra um exemplo do tipo de dados que o tipo de [!DNL Consents & Preferences] dados pode processar. As informações sobre o uso específico de cada um desses campos são fornecidas nas seções a seguir.
...
>[!NOTE]
>
>O exemplo acima serve para ilustrar a estrutura dos dados enviados [!DNL Platform] por meio do tipo de [!DNL Consents & Preferences] dados, a fim de contextualizar o restante desse documento, explicando os principais campos fornecidos pelo tipo de dados. O schema completo da estrutura do tipo de dados encontra-se no [apêndice](#full-schema) para efeitos de referência.
...
| --- | --- |
| `xdm:val` | A opção de consentimento fornecida pelo cliente para este caso de uso. Consulte o [apêndice](#choice-values) para ver os valores e definições aceites. |
...
>
>Pelo mesmo exemplo, se um cliente opt out do marketing direto para todos os canais (por meio `xdm:marketing`, explicado na [próxima seção](#marketing)), então esse cliente não deverá receber nenhuma mensagem, mesmo que a personalização seja permitida.
...
>[!IMPORTANT]
>
>Os consentimentos e preferências no nível do canal (ou seja, aqueles fornecidos `xdm:consents` fora do `xdm:idSpecific`) se aplicam às IDs dentro desse canal. Portanto, todos os consentimentos e preferências em nível de canal afetam diretamente se configurações equivalentes específicas de ID ou dispositivo são respeitadas:
>
...
| Propriedade | Descrição |
| --- | --- |
| `xdm:time` | Um carimbo de data e hora da última vez que qualquer um dos consentimentos e preferências do cliente foi atualizado. Esse campo pode ser usado em vez de aplicar carimbos de data e hora às preferências individuais para reduzir a carga e a complexidade. Fornecer um `xdm:time` valor sob uma preferência individual substitui o `xdm:metadata` carimbo de data e hora dessa preferência específica. |

## Como inserir dados usando o tipo de dados {#ingest}

Para usar o tipo de [!DNL Consents & Preferences] dados para assimilar dados de consentimento de seus clientes, você deve criar um conjunto de dados com base em um schema que contenha esse tipo de dados.

Consulte o tutorial sobre como [criar um schema na interface do usuário](http://www.adobe.com/go/xdm-schema-editor-tutorial-en) para obter etapas sobre como atribuir tipos de dados a campos. Depois de criar um schema contendo um campo com o tipo de [!DNL Consents & Preferences] dados, consulte a seção sobre como [criar um conjunto](../../catalog/datasets/user-guide.md#create) de dados no guia do usuário do conjunto de dados, seguindo as etapas para criar um conjunto de dados com um schema existente.
...
Quando um cliente altera seus consentimentos ou preferências em seu site, essas alterações devem ser coletadas e imediatamente aplicadas usando o SDK [da Web da](../../edge/consent/supporting-consent.md)Adobe Experience Platform. Se um cliente opt out da coleta de dados, toda a coleta de dados deverá cessar imediatamente. Se um cliente opt out da personalização, não deverá haver personalização presente na próxima página que visitar.

## Apêndice {#appendix}

As seções abaixo fornecem informações de referência adicionais sobre o tipo de [!DNL Consents & Preferences] dados.
...
| --- | --- | --- |
| `y` | Sim | O cliente opt in o consentimento ou preferência. Por outras palavras, **consentiram** na utilização dos seus dados, tal como indicado pelo consentimento ou preferência em questão. |
...
| `p` | Verificação pendente | O sistema ainda não recebeu o consentimento ou valor preferencial final. Isso é usado com mais frequência como parte de um consentimento que requer uma verificação em duas etapas. Por exemplo, se um cliente optar por receber e-mails, esse consentimento será definido como `p` até que ele selecione um link em um e-mail para verificar se ele forneceu o endereço de e-mail correto, para o qual o consentimento será atualizado `y`.<br><br>Se esse consentimento ou preferência não usar um processo de verificação de dois conjuntos, então a `p` escolha pode ser usada para indicar que o cliente ainda não respondeu ao prompt de consentimento. Por exemplo, você pode definir automaticamente o valor como `p` na primeira página de um site, antes que o cliente responda ao prompt de consentimento. Em jurisdições que não exigem consentimento explícito, você também pode usá-lo para indicar que o cliente não opt out explicitamente (ou seja, o consentimento é assumido). |
