...
| Recurso | Descrição |
| --- | --- |
| Calcular estatísticas em nível de coluna nos conjuntos de dados ADLS | A variável `ANALYZE TABLE` foi estendido com o `COMPUTE STATISTICS` Comando SQL. Agora é possível calcular estatísticas para um subconjunto de um conjunto de dados ADLS ou para determinadas colunas nesse conjunto de dados. Para obter mais informações, leia o [guia de computação de estatísticas do conjunto de dados](../../query-service/essential-concepts/dataset-statistics.md). |
