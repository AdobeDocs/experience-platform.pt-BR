...
keywords: adobe analytics;variáveis;análise;mapa automático;mapeado automaticamente;
...
# Variáveis mapeadas automaticamente no [!DNL Analytics]

Veja abaixo uma lista de variáveis que a Rede de borda da Adobe Experience Platform mapeia automaticamente para a Adobe Analytics. Informações detalhadas sobre os parâmetros de consulta da coleta de dados do Adobe Analytics podem ser encontradas no [Guia de implementação do Analytics](https://experienceleague.adobe.com/docs/analytics/implementation/validate/query-parameters.html?lang=pt-BR).

>[!NOTE]
>As informações nesta página também se aplicam ao SDK móvel do Adobe.
...
| commerce.order.currencyCode | cc | Mapeamento de MOEDA do parâmetro de consulta do AppMeasurement. |
| commerce.order.purchaseID | pi | Mapeamento PURCHASEID do parâmetro de consulta do AppMeasurement. |
...
| commerce.productListAdds.value | eventos | Mapeamento EVENT_LIST_FULL do parâmetro de consulta do AppMeasurement com conversão para COMMERCE_SC_ADD, usando delimitador `,`. |
...
| commerce.productListRemovals.value | eventos | Mapeamento EVENT_LIST_FULL do parâmetro de consulta do AppMeasurement com conversão para COMMERCE_SC_REMOVE, usando delimitador `,`. |
...
| commerce.productListViews.value | eventos | Mapeamento EVENT_LIST_FULL do parâmetro de consulta do AppMeasurement com conversão para COMMERCE_SC_VIEW, usando delimitador `,`. |
...
| commerce.productViews.value | eventos | Mapeamento EVENT_LIST_FULL do parâmetro de consulta do AppMeasurement com conversão para COMMERCE_PROD_VIEW, usando delimitador `,`. |
| commerce.purchases.value | eventos | Mapeamento EVENT_LIST_FULL do parâmetro de consulta do AppMeasurement com conversão para COMMERCE_PURCHASE, usando delimitador `,`. |
| device.colorDepth | c | Mapeamento C_COLOR do parâmetro de consulta do AppMeasurement. |
| device.screenHeight | s | Mapeamento de resolução de tela do parâmetro de consulta do AppMeasurement. |
| device.screenWidth | s | Mapeamento de resolução de tela do parâmetro de consulta do AppMeasurement. |
| environment.browserDetails.acceptLanguage | Accept-Language | Este é um mapeamento de cabeçalho HTTP, HEADER_ACCEPT_LANGUAGE. |
| environment.browserDetails.cookiesEnabled | k | Mapeamento de COOKIES do parâmetro de consulta AppMeasurement com conversão BOOLEAN_TO_YN. |
| environment.browserDetails.javaEnabled | v | Mapeamento JAVA_ENABLED do parâmetro de consulta do AppMeasurement com conversão BOOLEAN_TO_YN. |
...
| environment.browserDetails.viewportHeight | bh | Mapeamento BROWSER_HEIGHT do parâmetro de consulta AppMeasurement. |
| environment.browserDetails.viewportWidth | bw | Mapeamento BROWSER_WIDTH do parâmetro de consulta do AppMeasurement. |
| environment.connectionType | ct | Mapeamento CT_CONNECT_TYPE do parâmetro de consulta AppMeasurement. |
...
| media.mediaTimed.primaryAssetReference.showType | c.a.media.type | Dados de contexto do AppMeasurement `c.a.media.type` mapeamento com a conversão VIDEO_SHOW_TYPE. |
...
| placeContext.geo.longitude | lon | Mapeamento de LONGITUDE do parâmetro de consulta do AppMeasurement. |
| placeContext.geo.postalCode | CEP | Mapeamento ZIP do parâmetro de consulta AppMeasurement. |
| placeContext.geo.stateProvince | estado | Mapeamento STATE do parâmetro de consulta do AppMeasurement. |
| productListItems[N].lineItemId | produtos | Mapeamento da categoria de produtos do parâmetro de consulta do AppMeasurement. |
| productlistiitems[N].name | produtos | Parâmetro de consulta do AppMeasurement Mapeamento do nome de produto. |
| productlistiitems[N].priceTotal | produtos | Parâmetro de consulta do AppMeasurement Mapeamento de preço de produtos. |
| productlistiitems[N].quantidade | produtos | Parâmetro de consulta do AppMeasurement — Mapeamento da quantidade de produtos. |
| web.webInteraction.URL | pev1 | Mapeamento do parâmetro de consulta PAGE_EVENT_VAR1 do AppMeasurement. |
| web.webInteraction.name | pev2 | Mapeamento PAGE_EVENT_VAR2 do parâmetro de consulta do AppMeasurement. |
...
| web.webPageDetails.errorPage | pageType | Mapeamento PAGE_TYPE_FULL do parâmetro de consulta do AppMeasurement com conversão ERROR_PAGE_TYPE. |
| web.webPageDetails.homePage | hp | Mapeamento de parâmetro de consulta do AppMeasurement HOMEPAGE com conversão BOOLEAN_TO_YN. |
...
| web.webReferrer.URL | r | Mapeamento REFERRER do parâmetro de consulta do AppMeasurement. |
