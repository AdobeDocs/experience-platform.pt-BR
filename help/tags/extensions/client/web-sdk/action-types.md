...
1. **Renderizar ofertas de HTML de mbox**. Proposições solicitadas explicitamente por um escopo ou superfície de uma ação **[!UICONTROL Enviar evento]** não são renderizadas automaticamente. Você pode usar o tipo de ação **[!UICONTROL Aplicar propostas]** para informar ao SDK da Web onde renderizá-las especificando os metadados da proposta.
...
![Interface do usuário de Marcas de Plataforma mostrando o tipo de ação Aplicar Apresentações.](assets/apply-propositions.png)
...
* **[!UICONTROL Metadados de apresentação]**: um objeto que determina como as ofertas de HTML podem ser aplicadas. Essas informações podem ser fornecidas por meio do formulário ou por meio de um elemento de dados. Ele contém as seguintes propriedades:
   * **[!UICONTROL Escopo]**
   * **[!UICONTROL Seletor]**
...
Use o seletor **[!UICONTROL Instância]** para escolher a instância do SDK da Web que você deseja configurar. Se você tiver apenas uma instância, ela será pré-selecionada.

![Imagem da Interface do Usuário de Marcas da Plataforma mostrando as configurações da instância para o tipo de ação Enviar Evento.](assets/instance-settings.png)
...
![Imagem da Interface do Usuário de Marcas da Plataforma mostrando as configurações do elemento de dados para o tipo de ação Enviar Evento.](assets/data.png)
...
![Imagem da Interface do Usuário de Marcas da Plataforma mostrando as configurações do Personalization para o tipo de ação Enviar Evento.](assets/personalization-settings.png)
...
* **[!UICONTROL Solicitar personalização padrão]**: use esta seção para controlar se o escopo de toda a página (mbox global) e a superfície padrão (superfície da web com base na URL atual) são solicitados. Por padrão, isso é solicitado automaticamente durante a primeira chamada `sendEvent` do carregamento da página. Você pode escolher entre as seguintes opções:
   * **[!UICONTROL Automático]**: este é o comportamento padrão. Somente solicite a personalização padrão quando ainda não tiver sido solicitada. Isso corresponde a `requestDefaultPersonalization` não definido no comando do SDK da Web.
   * **[!UICONTROL Habilitado]**: solicitar explicitamente o escopo de página e a superfície padrão. Isso atualiza o cache de visualização do SPA. Isso corresponde a `requestDefaultPersonalization` definido como `true`.
