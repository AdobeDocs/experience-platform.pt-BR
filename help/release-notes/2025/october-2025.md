...
| Recurso | Descrição |
| --- | --- |
| Alerta de taxa de falha de ativação | Um novo alerta foi adicionado para destinos: **A taxa de falha de ativação excede o limite**. Esse alerta notifica quando o número de registros com falha durante a ativação de dados excedeu o limite permitido, permitindo que você responda rapidamente aos problemas de ativação. Leia a documentação sobre [regras padrão de alertas](../../observability/alerts/rules.md) para obter mais informações. |
