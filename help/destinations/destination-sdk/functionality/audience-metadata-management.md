...
# Gerenciamento de metadados do público-alvo
...
Você pode configurar a funcionalidade descrita neste documento usando o `/authoring/audience-templates` Ponto de extremidade da API. Ler [criar um modelo de metadados](../metadata-api/create-audience-template.md) para obter uma lista completa de operações que podem ser realizadas no ponto de extremidade.
...
![ID de mapeamento de entrada](../assets/functionality/input-mapping-id.png)
...
Você pode usar o template genérico para [criar um novo modelo de público-alvo](../metadata-api/create-audience-template.md) se a sua API suporta:
...
| `notify` | Aplica-se somente a destinos com base em arquivo. Inclui todos os componentes necessários (URL, método HTTP, cabeçalhos, solicitação e corpo de resposta) para fazer uma chamada HTTP para sua API, notificar você sobre exportações de arquivos bem-sucedidas. |

{style="table-layout:auto"}

### Exemplo de transmissão 1 {#example-1}
...
### Exemplo 2 de transmissão {#example-2}
...
### Exemplo de transmissão 3 {#example-3}
