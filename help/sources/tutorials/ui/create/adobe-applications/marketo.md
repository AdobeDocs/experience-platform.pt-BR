---
title: Criar uma conexão Marketo Engage Source e um fluxo de dados na interface
...
Na categoria *aplicativos de Adobe*, selecione **[!UICONTROL Marketo Engage]** e **[!UICONTROL Adicionar dados]**.
...
A página **[!UICONTROL Conectar conta Marketo Engage]** é exibida. Nesta página, você pode usar uma nova conta ou acessar uma conta existente.
...
* **[!UICONTROL Assimilação parcial]**: [Assimilação parcial de lotes](../../../../../ingestion/batch-ingestion/partial.md) é a capacidade de assimilar dados que contêm erros, até um determinado limite configurável. Esse recurso permite assimilar com sucesso todos os seus dados precisos no Experience Platform, enquanto todos os seus dados incorretos são armazenados em lote separadamente com informações sobre por que são inválidos.
...
O fluxo de dados de exemplo é uma configuração que você pode definir para seu fluxo de dados [!DNL Marketo] para limitar sua taxa de assimilação e, em seguida, experimentar recursos de Experience Platform sem precisar assimilar grandes quantidades de dados.
...
Ao seguir este tutorial, você criou com êxito um fluxo de dados para assimilar dados B2B da sua origem [!DNL Marketo Engage] para o Experience Platform.
...
A autenticação será negada se seu [!DNL Munchkin ID] não estiver mapeado para a organização de plataforma que você está usando. Configure o mapeamento entre o [!DNL Munchkin ID] e sua organização usando a [[!DNL Marketo] interface](https://app-sjint.marketo.com/#MM0A1).

![Uma mensagem de erro exibindo que a instância do Marketo não está mapeada corretamente para a organização Adobe.](../../../../images/tutorials/create/marketo/munchkin-not-mapped.png)
