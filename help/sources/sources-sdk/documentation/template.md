...
# Criar uma conexão *YOURSOURCE* usando a API [!DNL Flow Service]
...
Para conectar *YOURSOURCE* ao Experience Platform, você deve fornecer valores para as seguintes propriedades de conexão:
...
O tutorial a seguir orienta você pelas etapas para criar uma conexão de origem *YOURSOURCE* e criar um fluxo de dados para trazer dados *YOURSOURCE* para a Plataforma usando a [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

### Criar uma conexão básica {#base-connection}

Uma conexão base retém informações entre sua origem e a Platform, incluindo as credenciais de autenticação da origem, o estado atual da conexão e sua ID de conexão base exclusiva. A ID de conexão básica permite explorar e navegar pelos arquivos de dentro da origem e identificar os itens específicos que deseja assimilar, incluindo informações sobre os tipos de dados e formatos.

Para criar uma ID de conexão base, faça uma solicitação POST para o ponto de extremidade `/connections` enquanto fornece suas credenciais de autenticação da *YOURSOURCE* como parte do corpo da solicitação.
...
A solicitação a seguir cria uma conexão base para *YOURSOURCE*:
...
| `{SOURCE_PARAMS}` | Define parâmetros para o arquivo de origem que você deseja trazer para a Platform. Para recuperar o tipo de formato aceito para `{SOURCE_PARAMS}`, você deve codificar toda a cadeia de caracteres `list_id` em base64. No exemplo abaixo, `"list_id": "10c097ca71"` codificado em base64 equivale a `eyJsaXN0SWQiOiIxMGMwOTdjYTcxIn0=`. |
...
| `baseConnectionId` | A ID de conexão base de *YOURSOURCE*. Essa ID foi gerada em uma etapa anterior. |
| `connectionSpec.id` | A ID de especificação de conexão que corresponde à sua origem. |
| `data.format` | O formato dos dados de *YOURSOURCE* que você deseja assimilar. Atualmente, o único formato de dados com suporte é `json`. |
...
A solicitação a seguir cria uma conexão de destino para *YOURSOURCE*:
...
| `data.format` | O formato dos dados de *YOURSOURCE* que você deseja trazer para a Platform. |
...
A última etapa para trazer dados de *YOURSOURCE* para a Plataforma é criar um fluxo de dados. Até agora, você tem os seguintes valores necessários preparados:
