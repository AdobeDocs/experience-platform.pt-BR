---
description: Saiba como usar o Destination SDK para configurar um destino do Azure Blob Storage com opções de formatação de arquivo personalizadas e configuração de nome de arquivo personalizado.
title: Configure um destino do Azure Blob Storage com opções de formatação de arquivo personalizadas e configuração de nome de arquivo personalizado.
...
Esta página descreve como usar o Destination SDK para configurar um destino do [!DNL Azure Blob Storage] com [opções de formatação de arquivo](configure-file-formatting-options.md) personalizadas e [configuração de nome de arquivo](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration) personalizadas.

Esta página mostra todas as opções de configuração disponíveis para destinos do Azure Blob Storage. Você pode editar as configurações mostradas nas etapas abaixo ou excluir determinadas partes das configurações, conforme necessário.
...
A carga abaixo inclui uma configuração genérica do Azure Blob, com parâmetros de configuração personalizados de [formatação de arquivo CSV](../../functionality/destination-server/file-formatting.md) que os usuários podem definir na interface do usuário do Experience Platform.
...
Agora você sabe como criar um destino [!DNL Azure Blob Storage] personalizado usando o Destination SDK. Em seguida, sua equipe pode usar o [fluxo de trabalho de ativação para destinos baseados em arquivo](../../../ui/activate-batch-profile-destinations.md) para exportar dados para o destino.
