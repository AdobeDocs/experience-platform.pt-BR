---
keywords: Experience Platform;página inicial;tópicos populares;Conector de origem do Analytics;Conector do Analytics;origem do Analytics;análises
...
Selecionar **[!UICONTROL Próxima]** para continuar.

![cuidado](../../../../images/tutorials/create/analytics/caution.png)

#### Mapeamentos personalizados

Para usar funções de Preparo de dados e adicionar novo mapeamento ou campos calculados para atributos personalizados, selecione **[!UICONTROL Exibir mapeamentos personalizados]**.
...
Você pode usar a interface para navegar pela estrutura do esquema de origem e identificar o novo campo de origem que deseja usar. Depois de selecionar o campo de origem que deseja mapear, selecione **[!UICONTROL Selecionar]**.
...
Com o conjunto de mapeamento personalizado concluído, selecione **[!UICONTROL Próxima]** para continuar.
...
>abstract="Defina as regras de filtragem em nível de linha e coluna ao enviar dados para o Perfil do cliente em tempo real. Use a filtragem em nível de linha para aplicar condições e determinar quais dados devem ser **incluir para assimilação de perfil**. Use a filtragem em nível de coluna para selecionar as colunas de dados que deseja **excluir para assimilação de perfil**. As regras de filtragem não se aplicam aos dados enviados para o data lake."

>[!IMPORTANT]
>
>Suporte para filtragem [!DNL Analytics] os dados estão atualmente na versão beta e não estão disponíveis para todos os usuários. A documentação e a funcionalidade estão sujeitas a alterações.

Depois de concluir os mapeamentos para o [!DNL Analytics] conjunto de relatórios, você pode aplicar regras e condições de filtragem para incluir ou excluir seletivamente dados da assimilação para a variável [!DNL Profile Service]. O suporte para filtragem só está disponível para [!DNL Analytics] Os dados do e do são filtrados somente antes da entrada [!DNL Profile.] Todos os dados são assimilados no data lake.

#### Filtragem em nível de linha

>[!IMPORTANT]
>
>Use a filtragem em nível de linha para aplicar condições e determinar quais dados devem ser **incluir para assimilação de perfil**. Use a filtragem em nível de coluna para selecionar as colunas de dados que deseja **excluir para assimilação de perfil**.
