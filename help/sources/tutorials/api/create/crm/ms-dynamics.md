...
Uma conexão para a API especifica uma fonte e contém suas credenciais para essa fonte. Somente uma conexão para a API é necessária por [!DNL Dynamics] conta, pois pode ser usada para criar vários conectores de origem para trazer dados diferentes.
...
Ao seguir este tutorial, você criou uma conexão para sua [!DNL Dynamics] conta usando APIs e uma ID exclusiva foi obtida como parte do corpo da resposta. Você pode usar essa ID de conexão no próximo tutorial à medida que aprende a [explorar sistemas CRM usando a API](../../explore/crm.md)de Serviço de Fluxo.