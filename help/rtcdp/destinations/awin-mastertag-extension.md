...
description: A extensão Awin Advertiser Mastertag é um destino publicitário na Plataforma de dados do cliente em tempo real da Adobe. Para obter mais informações sobre a funcionalidade de extensão, consulte a página de extensão no Adobe Exchange.
...
A MasterTag é uma biblioteca JavaScript que contém todas as funções necessárias para a solução de rastreamento Awin e deve ser anexada incondicionalmente a cada página do site, incluindo a página de confirmação, mas excluindo qualquer página que exibe ou processa informações de pagamento.

Awin Advertiser Mastertag é uma extensão de publicidade na Plataforma de dados do cliente em tempo real da Adobe. Para obter mais informações sobre a funcionalidade de extensão, consulte a página de extensão no [Adobe Exchange](https://exchange.adobe.com/experiencecloud.details.103176.awin-advertiser-mastertag.html).
...
Para instalar a extensão Awin Advertiser Mastertag:
