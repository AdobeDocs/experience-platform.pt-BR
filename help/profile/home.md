...
### (Alfa) Atributos computados

>[!IMPORTANT]
>
>A funcionalidade de atributo calculado está em alfa. A documentação e a funcionalidade estão sujeitas a alterações.

Os atributos computados são funções usadas para agregar dados no nível do evento em atributos no nível do perfil. Essas funções são computadas automaticamente para que possam ser usadas na segmentação, ativação e personalização. Esses cálculos ajudam a responder facilmente a perguntas relacionadas a coisas como valor de compra vitalício, tempo entre compras ou número de aberturas de aplicativo, sem exigir que você realize cálculos complexos manualmente sempre que as informações forem necessárias. Para obter mais informações sobre atributos computados, incluindo a compreensão da função que os atributos computados desempenham no Adobe Experience Platform, comece lendo o [visão geral dos atributos computados](computed-attributes/overview.md).
