...
Depois de ler este artigo, você deve saber que o Experience Platform exporta automaticamente uma população histórica de todos os perfis que já se qualificaram para um público-alvo ativado quando o público-alvo é exportado pela primeira vez para o destino. Essa opção não é configurável no Destination SDK ou na interface do usuário do Experience Platform.
