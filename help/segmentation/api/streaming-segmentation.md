...
# Avaliar eventos em tempo quase real com a segmentação por transmissão
...
Você pode recuperar uma lista de todos os segmentos que estão ativados para segmentação por transmissão na sua organização IMS fazendo uma solicitação GET para a `/segment/definitions` terminal.
...
Uma resposta bem-sucedida retorna uma matriz de segmentos na Organização IMS que estão habilitados para segmentação por transmissão.
...
| `state` | *(Opcional)* Sequência de caracteres contendo o estado do agendamento. Valores disponíveis: `active` e `inactive`. O valor padrão é `inactive`. Uma Organização IMS só pode criar um agendamento. As etapas para atualizar o agendamento estão disponíveis posteriormente neste tutorial. |
