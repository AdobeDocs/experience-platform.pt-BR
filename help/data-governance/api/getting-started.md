...
`core` os recursos são os definidos e mantidos pela Adobe, enquanto `custom` os recursos são aqueles criados e mantidos pela sua organização e, portanto, são exclusivos e visíveis apenas para a sua organização. Como tal, operações de listagem e pesquisa (`GET`) são as únicas operações permitidas em `core` recursos, enquanto as operações de listagem, pesquisa e atualização (`POST`, `PUT`, `PATCH`, e `DELETE`) estão disponíveis para `custom` recursos.
