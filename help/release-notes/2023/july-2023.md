...
- [Notas de versão da Adobe Experience Platform](#adobe-experience-platform-release-notes)
   ...
   - [Serviço de consultas](#query-service-query-service)
...
| Atualização do [[!DNL Amazon S3] ](../../destinations/catalog/cloud-storage/amazon-s3.md#changelog) | Atualizado | Com esta atualização, o destino fornece funcionalidade aprimorada de exportação de arquivos e oferece suporte a exportações do conjunto de dados do [!BADGE Beta]{type=Informative}. |
| Atualização do [[!DNL Azure Blob] ](../../destinations/catalog/cloud-storage/azure-blob.md#changelog) | Atualizado | Com esta atualização, o destino fornece funcionalidade aprimorada de exportação de arquivos e oferece suporte a exportações do conjunto de dados do [!BADGE Beta]{type=Informative}. |
| Atualização do [[!DNL SFTP] ](../../destinations/catalog/cloud-storage/sftp.md#changelog) | Atualizado | Com esta atualização, o destino fornece funcionalidade aprimorada de exportação de arquivos e oferece suporte a exportações do conjunto de dados do [!BADGE Beta]{type=Informative}. |
| Conexão com o [[!DNL Adobe Campaign Managed Services] ](../../destinations/catalog/email-marketing/adobe-campaign-managed-services.md) | Atualizado | A integração do [!DNL Adobe Campaign Managed Services] com a Adobe Experience Platform agora é compatível com diferentes tipos de sincronização de público-alvo. Use o controle Selecionar tipo de sincronização para determinar se você deve exportar públicos-alvo para o Adobe Campaign ou os públicos-alvo e seus atributos de perfil. <br> ![Novo seletor Selecionar tipo de sincronização realçado.](/help/release-notes/2023/assets/acms-destination-export-type.png "Novo seletor Selecionar tipo de sincronização realçado."){width="100" zoomable="yes"} |
...
| Recurso | Descrição |
| --- | --- |
| Botão de alternância aprimorado no editor de consultas | O botão de alternância aprimorado do editor de consultas oferece melhor acessibilidade e suporte a vários temas. As configurações aprimoradas do editor permitem habilitar temas escuros ou claros. Consulte a [documentação da](../../query-service/ui/user-guide.md#enhanced-editor-toggle) para obter mais informações. |
| Nome de alias para estatísticas calculadas | Agora você pode fornecer um nome de alias para criar uma referência descritiva dos resultados de suas estatísticas calculadas em consultas SQL. Consulte a documentação para obter informações sobre esta e outras atualizações do comando COMPUTE STATISTICS. Consulte a [documentação da](../../query-service/key-concepts/dataset-statistics.md#alias-name) para obter mais informações. |
...
| --- | --- | --- |
| Classe | [[!UICONTROL XDM Individual Prospect Profile]](https://github.com/adobe/xdm/pull/1758/files) | Use esta classe para atrair perfis de clientes potenciais provenientes de casos de uso de conquista de cliente do topo de funil dos fornecedores de dados. |
| Grupo de campos | [[!UICONTROL Enriched Event Segment Details]](https://github.com/adobe/xdm/pull/1754/files) | Uma lista de públicos-alvo para os quais o perfil se qualifica no momento da coleta do evento. |
...
| --- | --- | --- |
| Grupo de campos | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de experimental para `stable`. |
| Grupo de campos | [[!UICONTROL Media Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `stable` para `deprecated`. |
| Tipo de dados | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Player state data information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Media event information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Media details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `stable` para `deprecated`. |
| Tipo de dados | [[!UICONTROL Custom metadata details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Chapter details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Advertising Pod details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Tipo de dados | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/pull/1756/files) | O `meta:status` foi atualizado de `experimental` para `stable`. |
| Extensão (gerenciamento de jornada do cliente) | [[!UICONTROL Domain]](https://github.com/adobe/xdm/pull/1756/files) | Campo `Domain` adicionado a [!UICONTROL Adobe CJM ExperienceEvent - Message Profile Details] para registrar o domínio do endereço de email do destinatário. |
| Extensão (gerenciamento de jornada do cliente) | [[!UICONTROL Channel's variant Name]](https://github.com/adobe/xdm/pull/1753/files) | Este campo foi adicionado a [!UICONTROL AJO Entity Fields] para representar o nome da variante de canal. |
| Extensão (Adobe Analytics) | [[!UICONTROL Context value]](https://github.com/adobe/xdm/pull/1761/files) | `Context value` foi adicionado a [!UICONTROL `Adobe Analytics ExperienceEvent Full Extension`]. |
