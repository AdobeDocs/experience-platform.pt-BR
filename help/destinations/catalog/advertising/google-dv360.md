...
* A ativação de preenchimentos retroativos do público-alvo para a [!DNL Google Display & Video 360] o destino está programado para ocorrer de 24 a 48 horas após um segmento ser mapeado pela primeira vez para uma conexão de destino. Essa atualização é uma resposta à política da Google de esperar 24 horas até assimilar dados e destina-se a melhorar as taxas de correspondência entre a CDP em tempo real e a [!DNL Google Display & Video 360]. Observe que essa é uma configuração de backend aplicável somente a esse destino e que não está relacionada a nenhuma opção de agendamento configurável pelo cliente na interface do usuário.
