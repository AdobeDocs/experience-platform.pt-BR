...
*Forneça uma breve visão geral para sua empresa, incluindo o valor que ela oferece aos clientes. Inclua um link para a página inicial da documentação do produto, para posterior leitura.*

>[!IMPORTANT]
>
>Esta página de documentação foi criada pela equipe *YOURDESTINATION*. Para quaisquer consultas ou solicitações de atualização, entre em contato diretamente com elas em *Inserir link ou endereço de email, onde você pode acessar para obter atualizações*
...
*Adicione informações nesta seção sobre as identidades suportadas pelo seu destino. Preenchemos previamente a tabela com alguns valores padrão. Exclua os valores que não se aplicam ao seu destino e quaisquer valores que não estejam pré-preenchidos.*

** YOURDESTINATIONsuporta a ativação de identidades descritas na tabela abaixo. Saiba mais sobre [identidades](https://experienceleague.adobe.com/docs/experience-platform/identity/namespaces.html?lang=en#getting-started).
...
| phone_sha256 | Números de telefone com hash com o algoritmo SHA256 | O Adobe Experience Platform oferece suporte para texto sem formatação e números de telefone com hash SHA256. Quando o campo de origem contém atributos sem hash, marque a opção **[!UICONTROL Apply transformation]** para ter [!DNL Platform] hash automaticamente os dados na ativação. |
...
**Exportar segmento**  - você está exportando todos os membros de um segmento (público-alvo) com os identificadores (nome, número de telefone ou outros) usados no destino  ** YOURDESTINATION.
...
*Uma plataforma residencial de aluguel e vendas deseja enviar notificações móveis para dispositivos Android e iOS de clientes para que saibam que há 100 listas atualizadas na área em que eles anteriormente procuravam por um aluguel.*
...
Para se conectar a esse destino, siga as etapas descritas no [tutorial de configuração de destino](https://experienceleague.adobe.com/docs/experience-platform/destinations/ui/connect-destination.html).

### Parâmetros de conexão {#parameters}

Enquanto [configurar](https://experienceleague.adobe.com/docs/experience-platform/destinations/ui/connect-destination.html) esse destino, você deve fornecer as seguintes informações:
...
* **[!UICONTROL ID]** da conta: A ID da  ** conta DESTINATION.
...
Leia [Ativar perfis e segmentos para os destinos de exportação de segmentos de transmissão](https://experienceleague.adobe.com/docs/experience-platform/destinations/ui/activate/activate-segment-streaming-destinations.html?lang=en) para obter instruções sobre como ativar segmentos de público-alvo para este destino.
