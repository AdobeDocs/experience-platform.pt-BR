...
# (Beta) Crie uma conexão de origem e um fluxo de dados para [!DNL SugarCRM Accounts & Contacts] uso da API do Serviço de fluxo
...
O tutorial a seguir guiará você pelas etapas para criar um [!DNL SugarCRM Accounts & Contacts] conexão de origem e criar um fluxo de dados para trazer [[!DNL SugarCRM]](https://www.sugarcrm.com/) dados de contas e contatos para a Adobe Experience Platform usando o [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Conectar [!DNL SugarCRM Accounts & Contacts] para a Platform usando o [!DNL Flow Service] API
...
Para criar um ID de conexão base, faça uma solicitação POST ao `/connections` ao fornecer sua [!DNL SugarCRM Accounts & Contacts] credenciais de autenticação como parte do corpo da solicitação.
...
A solicitação a seguir cria uma conexão básica para [!DNL SugarCRM Accounts & Contacts]:
...
| `{SOURCE_PARAMS}` | Define parâmetros para o arquivo de origem que você deseja trazer para a Platform. Para recuperar o tipo de formato aceito para `{SOURCE_PARAMS}`, você deve codificar toda a cadeia de caracteres em base64. <br> [!DNL SugarCRM Accounts & Contacts] O é compatível com várias APIs. Dependendo do tipo de objeto que você estiver utilizando, passe uma das opções abaixo: <ul><li>`accounts` : empresas com as quais sua organização tem um relacionamento.</li><li>`contacts` : pessoas com as quais sua organização tem um relacionamento estabelecido.</li></ul> |

A variável [!DNL SugarCRM Accounts & Contacts] O é compatível com várias APIs. Dependendo do tipo de objeto que você está usando, a solicitação a ser enviada é como a seguir:
...
Para [!DNL SugarCRM] API de contas exibe o valor de `{SOURCE_PARAMS}` é passado como `{"object_type":"accounts"}`. Quando codificado em base64, ele equivale a `eyJvYmplY3RfdHlwZSI6ImFjY291bnRzIn0=` conforme mostrado abaixo.
...
Para [!DNL SugarCRM] API de Contatos exibe o valor de `{SOURCE_PARAMS}` é passado como `{"object_type":"contacts"}`. Quando codificado em base64, ele equivale a `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` conforme mostrado abaixo.
...
Para [!DNL SugarCRM] API de contas o `object_type` o valor da propriedade deve ser `accounts`.
...
Para [!DNL SugarCRM] API de Contatos a `object_type` o valor da propriedade deve ser `contacts`.
...
| `data.format` | O formato do [!DNL SugarCRM Accounts & Contacts] dados que você deseja assimilar. No momento, o único formato de dados compatível é `json`. |
| `object_type` | [!DNL SugarCRM Accounts & Contacts] O é compatível com várias APIs. Dependendo do tipo de objeto que você estiver utilizando, passe uma das opções abaixo: <ul><li>`accounts` : empresas com as quais sua organização tem um relacionamento.</li><li>`contacts` : pessoas com as quais sua organização tem um relacionamento estabelecido.</li></ul> |
...
O último passo para trazer dados de [!DNL SugarCRM Accounts & Contacts] para a Platform é criar um fluxo de dados. Até agora, você tem os seguintes valores necessários preparados:
