...
A propriedade `edgeConfigId` é uma cadeia de caracteres que determina para qual [sequência de dados](../../../datastreams/overview.md) do Adobe Experience Platform você deseja enviar dados. Essa propriedade é necessária ao enviar dados para o Adobe.
...
Defina a propriedade de cadeia de caracteres `edgeConfigId` ao executar o comando `configure`. Essa propriedade é necessária para todas as implementações do SDK da Web. Se você omitir essa propriedade, o SDK da Web não saberá para qual fluxo de dados enviar dados, causando a perda permanente desses dados.
...
Se você configurar várias instâncias do SDK da Web em uma única página, deverá configurar um `edgeConfigId` diferente para cada instância.
