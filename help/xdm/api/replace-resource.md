...
description: O Registro de Schema permite que você substitua um recurso inteiro por meio de uma operação de PUT. Essa operação basicamente regrava o recurso, portanto, o corpo da solicitação deve incluir todos os campos que seriam necessários ao criar um novo recurso usando uma solicitação de POST.
