...
Use o formato de API a seguir para recuperar uma solicitação de publicação específica, definida pelo `{DESTINATION_ID}` parâmetro.
...
As duas solicitações a seguir recuperam todas as solicitações de publicação para sua Organização IMS ou uma solicitação de publicação específica, dependendo se você passa a variável `DESTINATION_ID` parâmetro na solicitação.
...
A solicitação a seguir recuperará a lista de solicitações de publicação enviadas, com base em [!DNL IMS Org ID] e a configuração da sandbox.
