...
# Crie um fluxo de dados para origens de protocolos Usando o [!DNL Flow Service] API

Este tutorial aborda as etapas para recuperar dados de uma origem de protocolos e trazê-los para a Platform usando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| `transformations.params.deltaColum` | A coluna designada usada para diferenciar entre dados novos e existentes. Os dados incrementais serão assimilados com base no carimbo de data e hora da coluna selecionada. O formato compatível com o `deltaColumn` ao usar OData genérico é `yyyy-MM-ddTHH:mm:ssZ`. |
