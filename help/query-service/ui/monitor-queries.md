...
Queries que usam blocos anônimos para compor suas instruções SQL são separados em suas subconsultas individuais. Isso permite inspecionar os detalhes de execução de cada bloco de query individualmente.

>[!NOTE]
>
>Os detalhes de execução de um bloco anônimo que usa o comando DROP serão **not** ser relatada como uma subconsulta separada. Detalhes de execução separados estão disponíveis para consultas CTAS, consultas ITAS e instruções COPY usadas como subconsultas de bloco anônimas. Os detalhes de execução do comando DROP não são suportados no momento.
...
As subconsultas de bloco anônimo têm guias à esquerda do status de execução. Selecione uma guia para exibir os detalhes da execução.
