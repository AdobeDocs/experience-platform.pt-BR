...
Você pode recuperar uma lista de todas as configurações de credenciais da organização fazendo uma solicitação de GET para a `/authoring/credentials` endpoint .
...
A solicitação a seguir recuperará a lista de configurações de credenciais às quais você tem acesso, com base na organização e na configuração da sandbox.
...
A resposta a seguir retorna o status HTTP 200 com uma lista de configurações de credenciais que você pode acessar, com base na ID da organização e no nome da sandbox usados. One `instanceId` corresponde ao template para uma configuração de credenciais. A resposta é truncada por brevidade.
