...
| Tipo de dados | [[!UICONTROL Informações de mídia]](https://github.com/lidiaist/xdm/blob/master/components/datatypes/media.schema.json) | Adição de um `xdm:playhead` para capturar o tempo do indicador de reprodução de um conteúdo de mídia. Validação de padrão fixo para `xdm:videoSegment`. |
