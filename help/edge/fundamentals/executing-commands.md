---
title: Executar comandos do Adobe Experience Platform Web SDK
description: Saiba como executar comandos do SDK da Web do Experience Platform
keywords: Executar comandos;commandName;Promise;getLibraryInfo;objetos de resposta;consentimento;
...
Se saber quando o comando é bem-sucedido não for importante para você, remova a chamada `then`.
...
Da mesma forma, se você não souber quando o comando falhar, poderá remover a chamada `catch`.
...
Todas as promessas retornadas de comandos são resolvidas com um objeto `result`. O objeto de resultado conterá dados dependendo do comando e do consentimento do usuário. Por exemplo, as informações da biblioteca são passadas como uma propriedade do objeto de resultados no seguinte comando.
