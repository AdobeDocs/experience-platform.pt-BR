...
| Origens | Você pode assimilar dados de várias fontes, como Soluções Adobe, armazenamento baseado em nuvem, software de terceiros e seu CRM. Consulte a seção [Fontes](#sources) abaixo para saber mais. |
...
[!DNL Destinations] são integrações pré-criadas com plataformas de destino suportadas pela Real-time Customer Data Platform do Adobe que ativam os dados para esses parceiros de forma contínua. Para obter mais informações, leia o artigo [Visão geral dos destinos](../../destinations/home.md).
...
O Adobe Experience Platform [!DNL Data Science Workspace] permite que cientistas de dados gerem insights de dados e conteúdo em aplicativos Adobe e sistemas de terceiros, criando e operacionalizando Modelos de Aprendizado de Máquina. O [!DNL Data Science Workspace] é totalmente integrado ao [!DNL Platform] e capacita o ciclo de vida completo da ciência de dados, incluindo a exploração e a preparação de dados XDM, seguidos pelo desenvolvimento e a operacionalização de Modelos para enriquecer automaticamente o [!DNL Real-Time Customer Profile] com Machine Learning Insights.

**Novos recursos**

| Recurso | Descrição |
| -----------| ---------- |
| Acesso a dados usando o SDK do [!DNL Platform] | Os blocos de anotações do iniciador e de Receitas pré-criados no [!DNL Python] agora usam o SDK [!DNL Platform] para acessar dados. |
...
| Segmentação de transmissão | O suporte para avaliação contínua de segmentos (segmentação por transmissão) permite que a maioria das regras de segmento sejam avaliadas, pois os dados estão sendo transmitidos para [!DNL Platform]. Esse recurso significa que a associação do segmento será atualizada sem a necessidade de executar trabalhos de segmentação programados. Algumas exceções se aplicam, como segmentos que usam relacionamentos de várias entidades ou com cargas úteis enriquecidas. |
...
| Suporte aprimorado para compartilhamento de público externo entre [!DNL Platform] e soluções Adobe | Os usuários agora podem trazer metadados de público-alvo externo (não [!DNL Experience Platform]) em cenários em que o número de públicos-alvo é grande ou não conhecido a priori. Esta versão inclui acesso aos metadados do [!DNL Audience Manager] para clientes que provisionaram o conector da solução. Esses metadados de público podem ser usados no Construtor de segmentos para criar novos [!DNL Experience Platform] segmentos. <br/><br/> Além disso, os segmentos criados em [!DNL Experience Platform] agora estarão disponíveis para uso em soluções de Adobe integradas, incluindo [!DNL Audience Manager], [!DNL Target] e [!DNL Ad Cloud]. |
