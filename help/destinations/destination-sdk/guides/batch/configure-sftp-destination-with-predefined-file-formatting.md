...
Esta página descreve como usar o Destination SDK para configurar um destino SFTP com predefinido e padrão [opções de formatação de arquivo](configure-file-formatting-options.md) e um [configuração do nome do arquivo](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).
...
A carga abaixo inclui uma configuração SFTP genérica, com predefinido e padrão [Formatação de arquivo CSV](../../functionality/destination-server/file-formatting.md) parâmetros de configuração que os usuários podem definir na interface do usuário do Experience Platform.
