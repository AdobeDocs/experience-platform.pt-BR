...
Cada evento tem seu próprio terminal. Todos os pontos de extremidade da API Media Edge são métodos POST, com corpos de solicitação JSON para dados de evento. Para obter mais informações sobre endpoints, parâmetros e exemplos da API do Media Edge, consulte a [Arquivo do Media Edge Swagger](swagger.md).

Este guia mostra como rastrear os seguintes eventos após iniciar a sessão:

* [Início do buffer](#buffer-start-event-request)
* [Reproduzir](#play-event-request)
* [Sessão concluída](#session-complete-event-request)

## Implementar a API {#implement-api}

Além de pequenas diferenças no modelo e nos caminhos chamados, a API Media Edge tem a mesma implementação que a [API da coleção de mídia](https://experienceleague.adobe.com/docs/media-analytics/using/implementation/streaming-media-apis/mc-api-overview.html). Os detalhes de implementação do Media Collection permanecem válidos para a API Media Edge, conforme descrito na documentação a seguir:

* [Definição do tipo de solicitação HTTP no seu reprodutor](https://experienceleague.adobe.com/docs/media-analytics/using/implementation/streaming-media-apis/mc-api-impl/mc-api-sed-pings.html)
* [Enviar eventos de ping](https://experienceleague.adobe.com/docs/media-analytics/using/implementation/streaming-media-apis/mc-api-impl/mc-api-sed-pings.html)
* [Condições de tempo limite](https://experienceleague.adobe.com/docs/media-analytics/using/implementation/streaming-media-apis/mc-api-impl/mc-api-timeout.html)
* [Controlar a ordem dos eventos](https://experienceleague.adobe.com/docs/media-analytics/using/implementation/streaming-media-apis/mc-api-impl/mc-api-ctrl-order.html)
...
## Início da sessão {#start-session}
...
## Solicitação de evento de início de buffer {#buffer-start}
...
Para obter mais informações sobre os parâmetros de endpoint do Início do buffer e exemplos, consulte a [Media Edge Swagger](swagger.md) arquivo.


## Reproduzir solicitação de evento {#play-event}
...
Para obter mais informações sobre parâmetros de endpoint de reprodução e exemplos, consulte [Media Edge Swagger](swagger.md) arquivo.

## Solicitação de evento de conclusão de sessão {#session-complete}
...
Para obter mais informações sobre parâmetros de endpoint de Sessão Concluída e exemplos, consulte [Media Edge Swagger](swagger.md) arquivo.
