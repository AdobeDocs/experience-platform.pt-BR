...
Você pode recuperar uma lista de todos os seus segmentos que são ativados para a segmentação de streaming na organização, fazendo uma solicitação GET para a variável `/segment/definitions` endpoint .
...
Uma resposta bem-sucedida retorna uma matriz de segmentos na sua organização que são ativados para a segmentação de streaming.
