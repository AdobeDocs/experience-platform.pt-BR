---
description: Esta página lista e descreve todas as operações de API que você pode executar usando o endpoint da API `/authoring/sample-profiles` para gerar perfis de amostra para usar no teste de destino.
title: Exemplos de operações de API de geração de perfil
...
# Exemplos de operações de API de geração de perfil {#sample-profile-api-operations}
...
>[!IMPORTANT]
>
>Adicionar os perfis de amostra gerados aqui a chamadas HTTP quando [testar o destino](./test-destination.md).
...
Para obter a ID de uma instância de destino, primeiro crie uma conexão na interface do usuário do Experience Platform com seu destino antes de tentar testar seu destino. Leia o [ativar tutorial de destino](/help/destinations/ui/activation-overview.md) e consulte a dica abaixo para obter a ID de instância de destinos para usar com essa API.

>[!TIP]
>
>* Obtenha a ID da instância de destino que você deve usar aqui no URL ao navegar por uma conexão com seu destino.
   >![Imagem da interface do usuário sobre como obter a ID da instância de destino](./assets/get-destination-instance-id.png)
...
>[!TIP]
>
>* A ID de destino que você deve usar aqui é a `instanceId` que corresponde a uma configuração de destino, criada usando o `/destinations` terminal. Consulte a [referência da API de configuração de destino](./destination-configuration-api.md#retrieve-list).
...
Depois de ler este documento, agora você sabe como gerar perfis de amostra a serem usados quando [teste de um template de transformação de mensagem](./create-template.md) ou quando [testando se o destino está configurado corretamente](./test-destination.md).
