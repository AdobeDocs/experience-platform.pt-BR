...
| --- | --- |
| `baseConnectionId` | A ID de uma conexão básica para um sistema de armazenamento em nuvem. |
...
>[!NOTE] Ao criar uma conexão de público alvo, certifique-se de usar o valor de conexão base Data Lake para a conexão básica `id` em vez da conexão base do conector de origem de terceiros.
...
Uma resposta bem-sucedida retorna os detalhes da especificação de fluxo de dados responsável por trazer os dados do seu armazenamento em nuvem para a Plataforma. Armazene o valor do `id` campo conforme necessário na próxima etapa para criar um novo fluxo de dados.
